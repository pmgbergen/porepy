...
def test_time_dependent_array():
    ...
    sd_prev_timestep = sd_array.at_previous_timestep()
    assert np.allclose(sd_prev_timestep.parse(mdg), 0)

    sd_top_prev_timestep = sd_array_top.at_previous_timestep()
    assert np.allclose(sd_top_prev_timestep.parse(mdg), 0)

    intf_prev_timestep = intf_array.at_previous_timestep()
    assert np.allclose(intf_prev_timestep.parse(mdg), np.arange(intf_val.size))

    bg_val_prev_timestep = bg_array.at_previous_timestep().parse(mdg)
    ...
    empty_prev_timestep = empty_array.at_previous_timestep()
    ...
    with pytest.raises(KeyError):
        _ = sd_prev_timestep.at_previous_timestep().parse(mdg)
...
def test_ad_variable_evaluation():
    ...
    prev_var_ad = var_ad.at_previous_timestep()
    ...
    v1_prev = v1.at_previous_timestep()
...
def test_ad_variable_prev_time_and_iter(prev_time):
    ...
    if prev_time:
        ...
        get_prev_key = 'at_previous_timestep'

        # prohibit prev time step variable to also be prev iter
        with pytest.raises(ValueError):
            var_pt = var.at_previous_timestep()
