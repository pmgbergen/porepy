...
def test_subface_mechanical_stress() -> None:
    """Check the assembly of mechanical forces on matrix subfaces in the 2D case.
    
    Actual values are obtained through the AD constitutive law machinery, while
    expected values are obtained by explicitky assembled from the subface MPSA
    matrices. 

    This test verifies that the selection of subface matrices, ordering of displacement
    variables, and vectorization of the mortar-to-matrix projection.

    Here, we consider a 2D domain with a single fracture, and a non-matching grid.
    Dirichlet boundary conditions for external boundaries are set to zero by default,
    and the displacement variables are set to non-zero values.
    """

    class SubfaceMomentumBalance(
        NonMatchingSquareDomainOrthogonalFractures,
        pp.MomentumBalance,
    ):
        """Momentum model using subface forces on a refined interface."""

        def use_subface_contact_force(self) -> bool:
            return True

    # Here the refinement is not established by explicit refinement of the fracture, but 
    # by using a non-matching grid. The consistency should be checked later.
    model = SubfaceMomentumBalance(
        {
            "fracture_indices": [0],
            "grid_type": "simplex",
    ...
    fracture = model.mdg.subdomains(dim=model.nd - 1)[0]
    interface = model.mdg.interfaces(dim=model.nd - 1)[0]

    topology = _fvutils.SubcellTopology(matrix)
    dim = model.nd

    # Assign distinguishable values to every component of every matrix
    # displacement degree of freedom. 
    matrix_displacement = np.vstack(
        (
            1.0 + np.arange(matrix.num_cells),
            101.0 + np.arange(matrix.num_cells),
        )
    ).ravel(order="F")

    # Assign distinguishable values to every component of every mortar cell.
    # Values are allowed to vary between mortar cells belonging to the same
    # primary full face.
    interface_displacement = np.vstack(
        (
            1001.0 + np.arange(interface.num_cells),
            2001.0 + np.arange(interface.num_cells),
        )
    ).ravel(order="F")
    ...
    # Verify that the external boundary contribution is zero. 
    external_boundary_operator = (
        model.combine_boundary_operators_mechanical_stress(
            [matrix]
        )
    )
    external_boundary_values = model.equation_system.evaluate(
        external_boundary_operator
    )

    np.testing.assert_allclose(
        external_boundary_values,
        0.0,
    )

    # Get the actual operator of the subface mechanical stress through
    # the constitutive law.
    actual_operator = model.subface_mechanical_stress(
        [matrix]
    )
    actual_operator.discretize(model.mdg)

    actual = model.equation_system.evaluate(
        actual_operator
    )

    # Calculate the subface MPSA matrices generated by the explicit
    # discretization.
    data = model.mdg.subdomain_data(matrix)
    matrices = data[
        pp.DISCRETIZATION_MATRICES
    ][model.stress_keyword]

    mpsa = pp.Mpsa(model.stress_keyword)

    subface_stress = matrices[
        mpsa.subface_stress_matrix_key
    ]
    subface_bound_stress = matrices[
        mpsa.subface_bound_stress_matrix_key
    ]

    # Construct the scalar mapping from matrix subfaces to mortar cells.
    scalar_subface_to_mortar = (
        _fvutils.map_subfaces_to_mortar(
            primary=matrix,
            secondary=fracture,
            interface=interface,
        )
    ...
    # Extend the scalar mapping from mortar cells to matrix subfaces to
    # vector quantities.
    mortar_to_subface = sps.kron(
        scalar_subface_to_mortar.T,
        sps.eye(dim),
        format="csr",
    )

    subface_boundary_values = (
        mortar_to_subface @ interface_displacement
    )

    cell_contribution = (
        subface_stress @ matrix_displacement
    )
    ...
    # Ensure that neither contribution vanishes.
    assert np.linalg.norm(cell_contribution) > 0.0
    assert np.linalg.norm(interface_contribution) > 0.0
