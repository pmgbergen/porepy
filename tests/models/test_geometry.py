...
from porepy.models.geometry import LoadGeometryMixin
...
@pytest.fixture(scope="module", params=geometry_list)
def geometry_class(request: pytest.FixtureRequest) -> type[pp.ModelGeometry]:
    return request.param


@pytest.fixture(scope="module")
def num_fracs(request: pytest.FixtureRequest) -> int:
    return request.param


# Set up geometry models for all tests but ``test_load_geometry_mixin``.
@pytest.fixture(scope="module")
def geometry_model(
    geometry_class: type[pp.ModelGeometry], num_fracs: int
) -> pp.ModelGeometry:
    """Fixture for setting up a :class:`~porepy.ModelGeometry` instance with parameters
    for the tests.

    Implementation note: We could also have a ``classmethod`` for this that fills a
    dictionary of instances of the geometry class (the dictionary would be an attribute
    of a test class). However, this solution allows more flexibility.

    Each geometry instance is configured with fracture indices and units, and its
    geometry is set.

    """
    # Create an instance of the geometry class.
    model = geometry_class()
    model.params = {"fracture_indices": [i for i in range(num_fracs)]}

    # Assign units to the geometry.
    model.units = pp.Units()
    ...
    model._operator_cache = {}

    return model


# ``test_load_geometry_mixin`` needs a factory method to create two geometry model
# instances. In addition, the geometry class need to be extended with
# ``LoadGeometryMixin`` and ``set_geometry`` should not be called yet.
@pytest.fixture(scope="function")
def geometry_model_with_loadgeometry_mixin_factory():
    """Fixture for defining a factory method that returns a fully set up
    :class:`~porepy.ModelGeometry` instance.

    """

    def make_geometry_model(
        geometry_class: type[pp.ModelGeometry], num_fracs: int, additional_params: dict
    ) -> pp.ModelGeometry:
        """Factory method for creating a geometry model instance.

        Parameters:
            geometry_class: Geometry class to instantiate.
            num_fracs: Number of fractures to set in the geometry.
            additional_params: Dictionary containing a ``"meshing_kwargs"`` and
            ``"grid_type"`` entry.
        ...
        """
        # Extend the geometry class with LoadGeometryMixin.
        geometry_class = models.add_mixin(LoadGeometryMixin, geometry_class)

        # Create an instance of the geometry class.
        model = geometry_class()
        model.params = {"fracture_indices": [i for i in range(num_fracs)]}

        # Add ``"meshing_kwargs"`` to model parameters.
        model.params.update(additional_params)

        # Assign units to the geometry.
        model.units = pp.Units()

        # The ``LoadGeometryMixin`` class creates the initial geometry in
        # ``create_and_export_geometry``. This is done in ``test_load_geometry_mixin``,
        # so we do not call ``set_geometry`` here.
...
@pytest.mark.parametrize("num_fracs", [1], indirect=True)
def test_set_geometry(geometry_model: pp.ModelGeometry) -> None:
    """Test the method set_geometry.

    Parameters:
        geometry_model: Geometry class instance to test.
    ...
    for attr in [
        ...
        assert hasattr(geometry_model, attr)


@pytest.mark.parametrize("num_fracs", [0, 3], indirect=True)
def test_boundary_sides(geometry_model: pp.ModelGeometry) -> None:
...
@pytest.mark.parametrize("num_fracs", [0, 3], indirect=True)
def test_wrap_grid_attributes(geometry_model: pp.ModelGeometry) -> None:
...
@pytest.mark.parametrize("num_fracs", [2], indirect=True)
def test_subdomain_interface_methods(geometry_model: pp.ModelGeometry) -> None:
...
@pytest.mark.parametrize("num_fracs", [0, 1, 2, 3], indirect=True)
def test_internal_boundary_normal_to_outwards(geometry_model: pp.ModelGeometry) -> None:
...
@pytest.mark.parametrize("num_fracs", [0, 1, 2, 3], indirect=True)
def test_outwards_normals(geometry_model: pp.ModelGeometry) -> None:
...
@pytest.mark.parametrize("num_fracs", [2], indirect=True)
def test_basis_normal_tangential_components(geometry_model: pp.ModelGeometry) -> None:
...
@pytest.mark.parametrize("num_fracs", [2], indirect=True)
def test_local_coordinates(geometry_model: pp.ModelGeometry) -> None:
...
def test_load_geometry_mixin(
    geometry_model_with_loadgeometry_mixin_factory: Callable,
    ...
    Test that a geometry can be created, exported to file, and loaded back
    correctly.
    ...
    params = {
        # The models in ``geometry_list`` all use simplex grids, but somehow this is
        # overriden in the ``LoadGeometryMixin``. Specify it here to be sure.
        "grid_type": "simplex",
        "meshing_kwargs": {
    ...
    model_1 = geometry_model_with_loadgeometry_mixin_factory(
        geometry_class, num_fracs, params
    )
    model_2 = geometry_model_with_loadgeometry_mixin_factory(
        geometry_class, num_fracs, params
    )

    # Create and export geometry for model 1, load stored geometry for model 2.
