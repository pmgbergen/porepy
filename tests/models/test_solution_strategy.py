...
@pytest.mark.parametrize(
    ...
    [
        ...
        (np.array([1e-6, 1e-6]), np.array([1e-6]), ConvergenceStatus.SUCCESS),
        ...
        (np.array([np.nan, 0.1]), np.array([1e-6]), ConvergenceStatus.FAILURE),
        # Case 5: Residual is above divergence tolerance.
        (np.array([1e-6, 1e-6]), np.array([2e4]), ConvergenceStatus.FAILURE),
    ],
)
