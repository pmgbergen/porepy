...
def test_pressure_and_fluxes():
    """ Check errors for pressure and flux solutions.
        ...
        The manufactured solution assumes unitary physical parameters. For the
        details about the solution setup, see Section 7.1 from [1].

        Tests should pass as long as the `desired_error` matches the `actual_error`,
        up to absolute (1e-5) and relative (1e-3) tolerances. The values of
        such tolerances aim at keeping the test meaningful while minimizing the chances
        of failure due to floating-point arithmetic close to machine precision.

        In this test, we are comparing errors for the pressure (for the rock and the
        fracture) and fluxes (for the rock, the fracture, and on the interface).
        The errors are measured in a discrete relative L2-error norm (such as the ones
        ...
        References:
            ...
            [2] Nordbotten, J. M. (2016). Stable cell-centered finite volume
                discretization for Biot equations. SIAM Journal on Numerical
                Analysis, 54(2), 942-968.
    ...
    np.testing.assert_allclose(
        actual_rock_pressure_error,
        desired_rock_pressure_error,
        atol=1e-5,
        rtol=1e-3
    )

    np.testing.assert_allclose(
        actual_rock_flux_error,
        desired_rock_flux_error,
        atol=1e-5,
        rtol=1e-3
    )

    np.testing.assert_allclose(
        actual_frac_pressure_error,
        desired_frac_pressure_error,
        atol=1e-5,
        rtol=1e-3
    )

    np.testing.assert_allclose(
        actual_frac_flux_error,
        desired_frac_flux_error,
        atol=1e-5,
        rtol=1e-3
    )

    np.testing.assert_allclose(
        actual_intf_flux_error,
        desired_intf_flux_error,
        atol=1e-5,
        rtol=1e-3
    )