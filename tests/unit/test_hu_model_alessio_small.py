...
class SolutionStrategyPressureMassTest(test_hu_model.SolutionStrategyPressureMass):
    ...
    def initial_condition(self) -> None:
            ...
            for sd in self.mdg.subdomains():
                ...
                pressure_values = np.ones(sd.num_cells) # ( 2 - 1 * sd.cell_centers[1] / self.ymax ) # np.ones(sd.num_cells)
    ...
    def after_nonlinear_iteration(self, solution_vector: np.ndarray) -> None:
        ...
        for sd in self.mdg.subdomains():
            if sd.dim == 2:
                pressure_2d = self.pressure([sd]).evaluate(self.equation_system).val

            if sd.dim == 1:
                pressure_1d = self.pressure([sd]).evaluate(self.equation_system).val    
        
        print("\npressure_2d = ", pressure_2d)
        print("pressure_1d = ", pressure_1d)
        ...
        # interface_mortar_flux_equation_phase_0: -------------------------------------
        ...
        eq.set_name("interface mortart phase 0 blabla")

        zeta_0 = eq.evaluate(self.equation_system).val
        print("\nzeta_0 = ", zeta_0)




        # interface_mortar_flux_equation_phase_1: -------------------------------------
        interfaces = self.mdg.interfaces()
        subdomains = self.interfaces_to_subdomains(interfaces)
        ...
        eq.set_name("interface mortart phase 1 blabla")

        zeta_1 = eq.evaluate(self.equation_system).val
        print("zeta_1 = ", zeta_1)


        # mob rho: ---------------------------------------------------------------
        phase = self.mixture.get_phase(1)
        discr = self.interface_ppu_discretization(interfaces, "interface_mortar_flux_phase_1")
        ...
        print("\nrho_mob_upwinded = ", rho_mob_upwinded)
        ...
        discr_intf_0 = self.interface_ppu_discretization(interfaces, "interface_mortar_flux_phase_0")
        discr_intf_1 = self.interface_ppu_discretization(interfaces, "interface_mortar_flux_phase_1")
        advected_entity = rho_mob 

        trace = pp.ad.Trace(subdomains)
        
        var_upwinded = (
            discr_intf_1.upwind_primary
            @ mortar_projection.primary_to_mortar_avg # should't it be int?
            @ trace.trace
            @ advected_entity
            + discr_intf_1.upwind_secondary
            @ mortar_projection.secondary_to_mortar_avg # should't it be int?
            @ advected_entity
        )

        up_prim_0 = discr_intf_0.upwind_primary.evaluate(self.equation_system)
        up_sec_0 = discr_intf_0.upwind_secondary.evaluate(self.equation_system)
        up_prim_1 = discr_intf_1.upwind_primary.evaluate(self.equation_system)
        up_sec_1 = discr_intf_1.upwind_secondary.evaluate(self.equation_system)
        
        var_upwinded = var_upwinded.evaluate(self.equation_system).val
        print("\nvar_upwinded = ", var_upwinded)



        # # interface_fluid_mass_flux_phase_1: -------------------------------------
        # phase = self.mixture.get_phase(1)
        # mortar = self.interface_mortar_flux_phase_1(interfaces)
        ...
        # ff_1 = flux.evaluate(self.equation_system).val
        # print("\nff_1 = ", ff_1)
        ...
        flux_intf_phase_0_p_val = -flux_intf_phase_0_p.evaluate(self.equation_system).val[[5,7]] # minus because of notation for mortars
        flux_intf_phase_1_p_val = -flux_intf_phase_1_p.evaluate(self.equation_system).val[[5,7]] # [upper face, lower face]
