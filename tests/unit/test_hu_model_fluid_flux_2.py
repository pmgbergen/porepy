...
class SolutionStrategyPressureMassTest(test_hu_model.SolutionStrategyPressureMass):
    ...
    def before_nonlinear_iteration(self):
        ...
        for intf, data in self.mdg.interfaces(return_data=True, codim=1):
            ...
            # sorry, I cant check the function interface_fluid_mass_flux_phase_0 in the model before running Newton. I copied it here:

            phase = self.mixture.get_phase(1)
            subdomains = self.interfaces_to_subdomains([intf])
            ...
            mortar_projection = pp.ad.MortarProjections(
                self.mdg, subdomains, [intf], dim=1
            )
            trace = pp.ad.Trace(subdomains)
            fluid_flux = pp.ad.DenseArray(mortar) * (
            ...
            fluid_flux = fluid_flux.evaluate(self.equation_system).val
            ...
            # assert np.all(fluid_flux == rho_mob * np.array([1, 1, -1, -1]))  # PASSED
            ...
            assert np.all(
                fluid_flux == rho_mob * np.array([0, 0, -0, -0])
            )  # phase 1, delta p = 0, g = 1, s_0 = 1
            assert np.all(
                fluid_flux
                == rho_mob
                * np.array([0.25, 0.25, -0.25, -0.25])  # remember that rho_1 = 0.5
            )  # phase 1, delta p = 0, g = 1, s_0 = 0
