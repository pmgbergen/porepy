    ...
    def solve(self, mdg, method=None):
        ...
        if method == "mvem":
            ...
            for g, d in mdg.subdomains(return_data=True):
                if g.dim == 2:
                    p_n[: g2d.num_cells] = d['stored_solutions']["pressure"][0][g.num_faces :]
                else:
                    p_n[g2d.num_cells :] = d['stored_solutions']["pressure"][0][g.num_faces :]
                d['stored_solutions']["pressure"][0] = d['stored_solutions']["pressure"][0][g.num_faces :]
            p = p_n
        return p
...
class TestMortar2DSimplexGrid(unittest.TestCase):
    ...
    def run_vem(self, mdg):
        ...
        self._solve(mdg, method, key)
        for g, d in mdg.subdomains(return_data=True):
            d["stored_solutions"]["darcy_flux"] = d['stored_solutions']["pressure"][0][: g.num_faces]
            d['stored_solutions']["pressure"][0] = d['stored_solutions']["pressure"][0][g.num_faces :]
