...
def setup():
    ...
    # Teardown
    shutil.rmtree(os.getcwd() + "/" + os.path.basename(os.path.normpath(setup.folder)))
...
def test_single_subdomains(setup):
    ...
    single_subdomains = [
        ...
        SingleSubdomain(
            pp.CartGrid(3, 1),
            f"{setup.folder_reference}/single_subdomain_1d.vtu",
        ),
        # 2d simplex grid
        SingleSubdomain(
            pp.StructuredTriangleGrid([3] * 2, [1] * 2),
            f"{setup.folder_reference}/single_subdomain_2d_simplex_grid.vtu",
        ),
        # 2d Cartesian grid
        SingleSubdomain(
            pp.CartGrid([4] * 2, [1] * 2),
            f"{setup.folder_reference}/single_subdomain_2d_cart_grid.vtu",
        ),
        # 2d polytopal grid
        SingleSubdomain(
            sd_polytop_2d,
            f"{setup.folder_reference}/single_subdomain_2d_polytop_grid.vtu",
        ),
        # 3d simplex grid
        SingleSubdomain(
            pp.StructuredTetrahedralGrid([3] * 3, [1] * 3),
            f"{setup.folder_reference}/single_subdomain_3d_simplex_grid.vtu",
        ),
        # 3d Cartesian grid
        SingleSubdomain(
            pp.CartGrid([4] * 3, [1] * 3),
            f"{setup.folder_reference}/single_subdomain_3d_cart_grid.vtu",
        ),
        # 3d polytopal grid
        SingleSubdomain(
            sd_polytop_3d,
            f"{setup.folder_reference}/single_subdomain_3d_polytop_grid.vtu",
        ),
    ]
