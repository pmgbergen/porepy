...
class TestContactMechanicsBiot:
    ...
    before_run_params = {"use_ad": True, "name": "before_run_default"}
    model_before_run = pp.ContactMechanicsBiot(before_run_params)
    model_before_run.prepare_simulation()

    after_run_params = {"use_ad": True, "name": "after_run_default"}
    model_after_run = pp.ContactMechanicsBiot(after_run_params)
    pp.run_time_dependent_model(model_after_run, after_run_params)

    @pytest.mark.parametrize("model", [model_before_run, model_after_run])
    def test_default_contact_mechanics_biot(self, model):
        ...
        if model.params["name"] == "before_run_default":
        ...
        np.testing.assert_equal(bc_mech_vals, np.array([0., 0., 0., 0., 0., 0., 0., 0.]))
        # Stiffness tensor
        ...
        np.testing.assert_equal(
            stiff.values,
            pp.FourthOrderTensor(np.array([1.0]), np.array([1.0])).values
        )
        # Source term
        ...
        np.testing.assert_equal(bc_flow_vals, np.array([0., 0., 0., 0.]))
        # Mass weights
        ...
        np.testing.assert_equal(
            perm,
            pp.SecondOrderTensor(np.array([1.0])).values
        )
        # Vector source
