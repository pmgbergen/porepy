...
class SolutionStrategyTest1(two_phase_hu.SolutionStrategyPressureMass):
    ...
    def deform_grid(self, mdg_ref=False):
        ...
        # aaa = g_old.nodes - g_new.nodes
        # bbb = g_old.cell_centers - g_new.cell_centers
        # ccc = g_old.cell_connection_map() - g_new.cell_connection_map()
        # ddd = g_old.cell_faces - g_new.cell_faces
        # eee = g_old.cell_nodes() - g_new.cell_nodes()
        # fff = g_old.cell_volumes - g_new.cell_volumes
        # ggg = g_old.face_areas - g_new.face_areas
        # hhh = g_old.face_centers - g_new.face_centers
        # iii = g_old.face_nodes - g_new.face_nodes
        # lll = g_old.get_all_boundary_faces() - g_new.get_all_boundary_faces()
        # mmm = g_old.get_boundary_faces() - g_new.get_boundary_faces()

        # for intf, data in self.mdg.interfaces(return_data=True):
        #     A_old = intf.primary_to_mortar_int().todense()
        #     B_old = intf.primary_to_mortar_avg().todense()

        #     C_old = intf.secondary_to_mortar_int().todense()
        #     D_old = intf.secondary_to_mortar_avg().todense()
        ...
        # for intf, data in self.mdg.interfaces(return_data=True):
        #     A_new = intf.primary_to_mortar_int().todense()
        #     B_new = intf.primary_to_mortar_avg().todense()

        #     C_new = intf.secondary_to_mortar_int().todense()
        #     D_new = intf.secondary_to_mortar_avg().todense()
        ...
        # print(np.sum(np.abs(A_old - A_new)))
        # print(np.sum(np.abs(B_old - B_new)))
        # print(np.sum(np.abs(C_old - C_new)))
        # print(np.sum(np.abs(D_old - D_new)))
