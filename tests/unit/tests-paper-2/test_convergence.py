...
class SolutionStrategyTest1(two_phase_hu.SolutionStrategyPressureMass):
    ...
    def after_simulation(self) -> None:
        ...
        if False:
            ...
            pp.plot_grid(sd, pressure_2d)
            pp.plot_grid(sd, saturation_2d)

            print("\n\nand now the reference one...")
            for sd in self.mdg_ref.subdomains():
                if sd.dim == 2:
                    break

            pressure_2d_projected = mapping_matrix_2d @ pressure_2d
            saturation_2d_projected = mapping_matrix_2d @ saturation_2d

            pp.plot_grid(sd, pressure_2d_projected)
            pp.plot_grid(sd, saturation_2d_projected)
