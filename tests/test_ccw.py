...
class TestCCW(unittest.TestCase):
    ...
    def test_tolerance(self):
        ...
        assert cg.is_ccw(p1, p2, p3, tol=1e-4, default=True)

        # Default kills us, even though we're inside safety margin
        assert not cg.is_ccw(p1, p2, p3, tol=1e-4, default=False)

        # Outside safety margin, and on the ccw side
        assert cg.is_ccw(p1, p2, p3, tol=1e-8, default=False)
    ...
    def test_tolerance_outside(self):
        ...
        assert not cg.is_ccw(p1, p2, p3, tol=1e-8, default=False)
