...
def test_compute_saturations(nphase: int, vectorized: int):
    ...
    with pytest.raises(ValueError):
        pc.compute_saturations(np.ones((nphase + 1, vectorized)), rho)
    with pytest.raises(ValueError):
        pc.compute_saturations(rho / nphase, np.ones((nphase, vectorized + 1)))

    # special case, single-phase is always saturated
    if nphase == 1:
        y = np.ones(shape)
        sat = pc.compute_saturations(y, rho)
...
def test_chainrule_fractional_derivatives():
    ...
    df_ext = pc.chainrule_fractional_derivatives(df, x_ext_v)
    ...
    df_0 = df[:, 0]
    assert df_0.shape == (5,)
    df_ext_0 = pc.chainrule_fractional_derivatives(df_0, x_ext)
