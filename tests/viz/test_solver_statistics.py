...
def test_log_simulation_status(statuses):
    ...
    for s in statuses:
        ...
        stats.advance()  # Used after each iteration.
...
def test_solver_statistics_advance():
    """Test advancing SolverStatistics."""
    stats = SolverStatistics()
    stats.counter = 5
    stats.custom_data = {"foo": "bar"}
    stats.advance()
    assert stats.counter == 6
    assert stats.custom_data == {}
...
def test_solver_statistics_append_custom_data():
    ...
    # Need to make sure that the data dict has the correct 'counter' keys before
    ...
    reference_data.pop("global")  # Only test custom data here.
...
def test_nonlinear_solver_statistics_advance_iteration():
    """Tests advancing iteration of NonlinearSolverStatistics."""
    stats = NonlinearSolverStatistics()
    stats.advance_iteration()
    assert stats.num_iteration == 1
    stats.advance_iteration()
    assert stats.num_iteration == 2
...
def test_nonlinear_solver_statistics_advance():
    """Tests advancing NonlinearSolverStatistics."""
    stats = NonlinearSolverStatistics()
    stats.num_iteration = 2
    stats.convergence_status = {
...
def test_nonlinear_solver_statistics_append_iterative_data():
    ...
    # Make sure that the data dict has the correct 'counter' key before appending.
...
def test_nonlinear_solver_statistics_append_data():
    ...
    # Advance counter and retry second outer iteration.
    stats.advance()
...
def test_time_statistic_attributes():
    """Tests initialized statistics in model integration."""
...
def test_time_statistics_advance():
...
def test_time_statistics_append_iterative_data():
    ...
    assert DeepDiff(out, reference_data, ignore_numeric_type_changes=True) == {}
...
def test_nonlinear_solver_and_time_statistics_append_data():
    ...
    # Advance counter and retry second time step iteration.
    stats.advance()
