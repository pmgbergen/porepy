...
def reference_solver_statistics_dict() -> dict:
    ...
    return {
        ...
        "global": {
            ...
            "final_simulation_status": SolverStatus.SUCCESSFUL,
            "num_entries": 2,
        },
        # Custom data from SolverStatistics
        "0": {"foo": ["bar1", "bar2"], "simulation_status": SolverStatus.FAILED},
        "1": {"foo": ["bar3"], "simulation_status": SolverStatus.SUCCESSFUL},
    }
    ...
    reference_dict["0"].update(
        {
            ...
            "num_iterations": 2,
            "simulation_status": SolverStatus.FAILED,
            "convergence_status": {
                "crit1": [
...
def test_solver_statistics_initialization():
    ...
    assert stats.simulation_status == SolverStatus.IN_PROGRESS
...
@pytest.mark.parametrize(
    "statuses",
    [
        [SolverStatus.IN_PROGRESS, SolverStatus.SUCCESSFUL],
        [
            SolverStatus.FAILED,
            SolverStatus.SUCCESSFUL,
...
def test_solver_statistics_append_iterative_data():
    ...
    stats = SolverStatistics()
    stats.increase_index()
    stats.simulation_status = SolverStatus.FAILED
    data = {"0": {}}
    out = stats.append_iterative_data(data)
    assert (
        DeepDiff(
            out,
            {"0": {"simulation_status": SolverStatus.FAILED}},
            ignore_string_type_changes=True,
        )
        == {}
    )
...
def test_nonlinear_solver_statistics_increase_index():
    ...
    stats.convergence_status = {
        "crit1": [SolverStatus.SUCCESSFUL, SolverStatus.FAILED],
        "crit2": [SolverStatus.FAILED, SolverStatus.SUCCESSFUL],
    }
    stats.convergence_info = {
        "crit1": [1.0, 0.5],
    }
...
def test_time_statistics_append_iterative_data():
    ...
    stats.index = 1
    stats.simulation_status = SolverStatus.SUCCESSFUL
    stats.time_index = 1
    stats.time = 2.5
    stats.dt = 0.5
    stats.final_time_reached = True
