...
class PolygonSegmentIntersectionTest(unittest.TestCase):
    ...
    def test_one_intersection(self):
        ...
        p_1_2 = basics.polygon_boundaries_intersect(p_1, p_2)
        ...
        p_2_1 = basics.polygon_boundaries_intersect(p_2, p_1)
    ...
    def test_mutual_intersection(self):
        ...
        p_1_3 = basics.polygon_boundaries_intersect(p1, p3)
        ...
        p_3_1 = basics.polygon_boundaries_intersect(p3, p1)
        p_i_known_2 = np.array([0, 0, 1.0])
    ...
    def test_parallel_planes(self):
        ...
        isect = basics.polygon_boundaries_intersect(p_1, p_2)
    ...
    def test_segments_intersect(self):
        # Test where the planes intersect in a way where segments only touches segments, which does not qualify as intersection
        p_1, _, _, p_4, *rest = self.setup_polygons()
        isect = basics.polygon_boundaries_intersect(p_1, p_4)
        assert isect is None
        # Also try the other way around
        isect = basics.polygon_boundaries_intersect(p_4, p_1)
        assert isect is None
    ...
    def test_segments_same_plane_no_isect(self):
        ...
        isect = basics.polygon_boundaries_intersect(p_1, p_2)
        assert isect is None
        isect = basics.polygon_boundaries_intersect(p_2, p_1)
        assert isect is None
        
    def test_segments_same_plane_isect(self):
        # Polygons in the same plane, and intersection. Should raise an exception
        p_1, *rest = self.setup_polygons()
