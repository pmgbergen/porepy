...
class Test2dDomain:
    ...
    def _generate_mesh(self, pts, edges, constraints=None):
        ...
        mdg = network.mesh_with_gmsh(self.mesh_args, constraints=constraints)
        return mdg
...
class TestGmshTags:
    ...
    def test_boundary(self):
        ...
        mdg = network.mesh_with_gmsh(mesh_args)
    ...
    def test_single_constraint(self, mesh_args):
        ...
        mesh_args = {"mesh_size_frac": 1}
        mdg = network.mesh_with_gmsh(mesh_args, constraints=np.array([0]))
        g = mdg.subdomains(dim=2)[0]
    ...
    def test_two_constraints(self, mesh_args):
        ...
        mdg = network.mesh_with_gmsh(mesh_args, constraints=np.arange(2))
        g = mdg.subdomains(dim=2)[0]
    ...
    def test_fracture_and_constraints(self):
        ...
        mesh_args = {"mesh_size_frac": 1}
        mdg = network.mesh_with_gmsh(mesh_args, constraints=constraints)
        g = mdg.subdomains(dim=2)[0]
