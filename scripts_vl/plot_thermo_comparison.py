...
results_file = 'data/results/pr_result_VL_reg-smoother.csv'
identifier_file = 'data/results/pr_result_VL_reg-smoother_ID.csv'
...
result_fnam_stripped = results_file[results_file.rfind('/') + 1: results_file.rfind('.csv')]
...
def plot_overshoot(axis, vals, T, p, name):
    ...
    if np.any(under_shoot):
        img_u = axis.plot(T[under_shoot].flat, p[under_shoot].flat,
            'v', markersize=3, color = 'black'
        )
        legend_img.append(img_u[0])
        legend_txt.append(f"{name} < 0")
    if np.any(over_shoot):
        img_o = axis.plot(T[over_shoot].flat, p[over_shoot].flat,
...
fig.tight_layout()
fig.savefig(f"{str(path)}/{figure_path}1_success_and_regions__{result_fnam_stripped}.png", format="png", dpi=500)
fig.show()
...
y_err_mesh[np.isnan(y_err_mesh)] = 0.
y_mesh[np.isnan(y_mesh)] = 0.
...
axis = fig.add_subplot(gs[0, 0])
img = axis.pcolormesh(
    T_mesh, p_mesh_f, y_mesh, cmap=cmap, norm=norm,
...
cb = fig.colorbar(img, cax=cax, orientation="vertical",
    ticks=ticks,
    # format=ticker.LogFormatterSciNotation(linthresh=linthresh, labelOnlyBase=False, minor_thresholds=(2, 0.4))
)

axis = fig.add_subplot(gs[0, 1])
img = axis.pcolormesh(
    T_mesh, p_mesh_f, y_err_mesh, cmap="Greys",
    vmin=y_err_mesh.min(), vmax=y_err_mesh.max(), shading='nearest'
)
...
cb = fig.colorbar(img, cax=cax, orientation="vertical")
cb.set_label(
    "Max abs. error: " + "{:.0e}".format(float(y_err_mesh.max()))
    + "\nL2-error: " + "{:.0e}".format(float(np.sqrt(np.sum(np.square(y_err_mesh)))))
)

fig.tight_layout()
fig.savefig(f"{str(path)}/{figure_path}2_gas_fraction__{result_fnam_stripped}.png", format="png", dpi=500)
fig.show()

# endregion

# region Plot 3: Absolute error in phase compositions
...
# filter out nans where the flash failed
x_h2o_L_err_mesh[np.isnan(x_h2o_L_err_mesh)] = 0.
x_co2_L_err_mesh[np.isnan(x_co2_L_err_mesh)] = 0.
x_h2o_G_err_mesh[np.isnan(x_h2o_G_err_mesh)] = 0.
x_co2_G_err_mesh[np.isnan(x_co2_G_err_mesh)] = 0.
...
axis = fig.add_subplot(gs[0, 0])
img = axis.pcolormesh(
    T_mesh, p_mesh_f, x_h2o_L_err_mesh, cmap="Greys",
    vmin=x_h2o_L_err_mesh.min(), vmax=x_h2o_L_err_mesh.max(), shading='nearest'
)
img_c, leg_c = plot_crit_point(axis)
axis.legend(
    img_c,
...
cb = fig.colorbar(img, cax=cax, orientation="vertical")
cb.set_label(
    "Max abs. error: " + "{:.0e}".format(float(x_h2o_L_err_mesh.max()))
    + "\nL2-error: " + "{:.0e}".format(float(np.sqrt(np.sum(np.square(x_h2o_L_err_mesh)))))
)

axis = fig.add_subplot(gs[0, 1])
img = axis.pcolormesh(
    T_mesh, p_mesh_f, x_co2_L_err_mesh, cmap="Greys",
    vmin=x_co2_L_err_mesh.min(), vmax=x_co2_L_err_mesh.max(), shading='nearest'
)
img_c, leg_c = plot_crit_point(axis)
axis.legend(
    img_c,
...
cb = fig.colorbar(img, cax=cax, orientation="vertical")
cb.set_label(
    "Max abs. error: " + "{:.0e}".format(float(x_co2_L_err_mesh.max()))
    + "\nL2-error: " + "{:.0e}".format(float(np.sqrt(np.sum(np.square(x_co2_L_err_mesh)))))
)

axis = fig.add_subplot(gs[1, 0])
img = axis.pcolormesh(
    T_mesh, p_mesh_f, x_h2o_G_err_mesh, cmap="Greys",
    vmin=x_h2o_G_err_mesh.min(), vmax=x_h2o_G_err_mesh.max(), shading='nearest'
)
img_c, leg_c = plot_crit_point(axis)
axis.legend(
    img_c,
...
cb = fig.colorbar(img, cax=cax, orientation="vertical")
cb.set_label(
    "Max abs. error: " + "{:.0e}".format(float(x_h2o_G_err_mesh.max()))
    + "\nL2-error: " + "{:.0e}".format(float(np.sqrt(np.sum(np.square(x_h2o_G_err_mesh)))))
)

axis = fig.add_subplot(gs[1, 1])
img = axis.pcolormesh(
    T_mesh, p_mesh_f, x_co2_G_err_mesh, cmap="Greys",
    vmin=x_co2_G_err_mesh.min(), vmax=x_co2_G_err_mesh.max(), shading='nearest'
)
img_c, leg_c = plot_crit_point(axis)
axis.legend(
    img_c,
...
cb = fig.colorbar(img, cax=cax, orientation="vertical")
cb.set_label(
    "Max abs. error: " + "{:.0e}".format(float(x_co2_G_err_mesh.max()))
    + "\nL2-error: " + "{:.0e}".format(float(np.sqrt(np.sum(np.square(x_co2_G_err_mesh)))))
)

fig.tight_layout()
fig.savefig(f"{str(path)}/{figure_path}3_phase_composition_error__{result_fnam_stripped}.png", format="png", dpi=500)
fig.show()
# endregion

# region Plot 4: Duality gap
...
dual_gap_L_mesh[np.isnan(dual_gap_L_mesh)] = 0.
dual_gap_G_mesh[np.isnan(dual_gap_G_mesh)] = 0.
...
axis = fig.add_subplot(gs[0, 0])
img = axis.pcolormesh(
    T_mesh, p_mesh_f, dual_gap_L_mesh, cmap="Greys",
    vmin=dual_gap_L_mesh.min(), vmax=dual_gap_L_mesh.max(), shading='nearest'
)
img_c, leg_c = plot_crit_point(axis)
axis.legend(
    img_c,
...
axis = fig.add_subplot(gs[0, 1])
img = axis.pcolormesh(
    T_mesh, p_mesh_f, dual_gap_G_mesh, cmap="Greys",
    vmin=dual_gap_G_mesh.min(), vmax=dual_gap_G_mesh.max(), shading='nearest'
)
img_c, leg_c = plot_crit_point(axis)
axis.legend(
    img_c,
...
fig.tight_layout()
fig.savefig(f"{str(path)}/{figure_path}4_duality_gap__{result_fnam_stripped}.png", format="png", dpi=500)
fig.show()
