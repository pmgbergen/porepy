...
class TestModel(pp.models.abstract_model.AbstractModel):
    ...
    def __init__(self, params: dict) -> None:
        ...
        self.initial_sw: float = 0.0
        self.initial_sn: float = 1.0
        ...
        self.rho_w: float = 1.0
        self.rho_n: float = 1.0
    ...
    def prepare_simulation(self) -> None:
        ...
        self._prolong_system = self.dof_manager.projection_to(
            self._system_vars
        ).transpose()
    ...
    def _set_up(self) -> None:
        ...
        for sd, data in self.mdg.subdomains(return_data=True):
            ...
            transmissibility = pp.SecondOrderTensor(
                np.ones(sd.num_cells) * self.permeability
            )
            pp.initialize_data(
                sd,
                data,
    ...
    def before_newton_iteration(self) -> None:
        ...
        for sd, data in self.mdg.subdomains(return_data=True):
            ...
            data["parameters"][f"{self.upwind_keyword}_sw"]["darcy_flux"] = np.copy(
                flux
            )
            data["parameters"][f"{self.upwind_keyword}_sn"]["darcy_flux"] = np.copy(
                flux
            )
    ...
    def assemble_and_solve_linear_system(self, tol: float) -> np.ndarray:
        ...
        if self._use_pressure_equation:
            A, b = self.ad_system.assemble_subsystem(
                variables=self.system["primary_vars"]
            )
        else:
            if self._monolithic:
                ...
                A_pp, b_p = self.ad_system.assemble_subsystem(
                    self.system["primary_equations"], self.system["primary_vars"]
                )
                A_sp, _ = self.ad_system.assemble_subsystem(
                    self.system["secondary_equations"], self.system["primary_vars"]
                )
                A_ps, _ = self.ad_system.assemble_subsystem(
                    self.system["primary_equations"], self.system["secondary_vars"]
                )
                A_ss, b_s = self.ad_system.assemble_subsystem(
                    self.system["secondary_equations"], self.system["secondary_vars"]
                )
    ...
    def set_unity(self) -> None:
        ...
        time_derivative = [
            self.sw
            - self.sw.previous_timestep()
            + self.sn
            - self.sn.previous_timestep()
        ]
        unity -= self.sw
        unity -= self.sn
    ...
    def set_satur_equation(self) -> None:
        ...
        accumulation = (
            self.mass_matrix.mass * self.rho_w * (self.sw - self.sw.previous_timestep())
        )
        ...
        accumulation = (
            self.mass_matrix.mass * self.rho_n * (self.sn - self.sn.previous_timestep())
        )
