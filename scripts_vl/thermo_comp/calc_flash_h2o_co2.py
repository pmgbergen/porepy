...
MIX, ADS, FLASH = get_MIX_AD_FLASH(num_cells)
...
if vectorize:
    ...
    y = list(GAS.fraction.evaluate(ADS).val)
    x_h2o_L = list(LIQ.fraction_of_component(H2O).evaluate(ADS).val)
    x_co2_L = list(LIQ.fraction_of_component(CO2).evaluate(ADS).val)
    x_h2o_G = list(GAS.fraction_of_component(H2O).evaluate(ADS).val)
    x_co2_G = list(GAS.fraction_of_component(CO2).evaluate(ADS).val)
else:
