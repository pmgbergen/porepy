...
for i in range(n):
    for j in range(m):
        ...
        if (
            abs(a_ij - pp.composite.A_CRIT) < a_eps
            and abs(b_ij - pp.composite.B_CRIT) < b_eps
        ):
...
img_liq = ax_liq.pcolormesh(
    A, B, RES_LIQ, cmap="Greys", vmin=RES_LIQ.min(), vmax=RES_LIQ.max()
)
...
cb_liq.set_label(
    "Log(Rel(abs(residual)))\nMax error: " + "{:.0e}".format(float(MAX_ERR_LIQ))
)

ax_liq.plot(
    [0, pp.composite.A_CRIT], [0, pp.composite.B_CRIT], color="red", linewidth=1
)
...
img_gas = ax_gas.pcolormesh(
    A, B, RES_GAS, cmap="Greys", vmin=RES_GAS.min(), vmax=RES_GAS.max()
)
...
fig.tight_layout()
fig.savefig("/mnt/c/Users/vl-work/Desktop/Z_calcs.png", format="png", dpi=500)
fig.show()
