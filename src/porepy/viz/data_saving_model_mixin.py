...
class DataSavingMixin(pp.PorePyModel):
    ...
    def save_data_time_step(self) -> None:
        ...
        if (
            not self.is_time_dependent() and self.nonlinear_solver_statistics.index >= 0
        ):  # stationary problem (avoid storing initial condition).
...
class IterationExporting(pp.PorePyModel):
    ...
    def after_solver_iteration(self, solution_vector: np.ndarray) -> None:
        ...
        """
        super().after_solver_iteration(solution_vector)  # type: ignore[safe-super]
