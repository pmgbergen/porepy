...
def setup_csv_reader(file_name: Path):
    """Return the"""
    with open(file_name, "r") as csv_file:
        ...
        # Returning here closes the file which will raise an error.
        return spam_reader.read(), has_header
...
def validate_csv_file(file_name: Path) -> None:
    """Assert that a file is in valid csv format.

    Raises:
        ValueError: If any two lines have a different number of columns.

    """
    spam_reader, _ = setup_csv_reader(file_name)
    for i, line in enumerate(spam_reader):
        # Infer the number of columns and assert that it's equal for all lines.
        if i == 0:
...
def network_from_csv_old(
    ...
    nd = infer_network_dimension(file_name)

    if nd == 2:
        return_value = _network_2d_from_csv(file_name, has_domain, tol=tol, **kwargs)
        if isinstance(return_value, FractureNetwork2d):
            return return_value
        else:
            return return_value[0]
    else:
        return _network_3d_from_csv(file_name, has_domain, tol, **kwargs)
    # QUESTION What do we do about empty csv files without a domain line? They can be
    # be generated by `FractureNetwork.to_csv`. Which dimension does the network have?
    # Raise a ValueError in that case?


def _network_3d_from_csv(
    file_name: Path, has_domain: bool = True, tol: float = 1e-4, **kwargs
) -> FractureNetwork3d:
