...
#
#    cells_l, faces_l, rm_vectors, actual_rm, normal_rm \
#        = identify_correlation_points(gb, g_h, g_l, rm, u, f_c)
#
#    delta_u = relative_displacements(u, f_c, g_l, cells_l, faces_l, g_h)
#    # Use them to compute the SIFs locally
#    if kw.get('use_normal_distances', False):
#        rm_distances = normal_rm
#    else:
#        rm_distances = actual_rm
#    stress_intensity_factors \
#        = sif_from_delta_u(delta_u, rm_distances, mu, kappa)
#
...
#
#
#def identify_correlation_points(gb, g_h, g_l, rm, u, f_c):
#    """
...
#    # Lower-dimensional cell containing the correlation point (as its cell
#    # center).
#    cells_l = []
#    # Distances from the above cell centers to each fracture tip face center.
#    actual_rm = []
#    # Normal distance between the cell center and the fracture front (defined
#    # by the face of the fracture tip).
#    normal_rm = []
...
#            normal_dist, _ = comp_geom.dist_points_segments(
...
def relative_displacements(u, f_c, g_l, cells_l, faces_l, g_h):
    ...
    for i in range(n_tips):
        ...
        if g_h.dim == 3:
            ...
            # with x
            tangent_l = g_l.cell_faces[face_l].data * g_l.face_normals[:, face_l]
