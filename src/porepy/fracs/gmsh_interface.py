...
class Tags(Enum):
    ...
    These may be mapped to the string-based tag system used in Gmsh
    (see :class:`PhysicalNames`).
    ...
    NEUTRAL = 0
    """A neutral tag. Objects with this tag will not be accounted for in any particular
...
class PhysicalNames(Enum):
    ...
    The mesh generated by gmsh will use these to tag cells on geometric objcets.
...
class _GmshData:
    ...
    The third row should contain tags for the lines
    (note that these are needed for all lines, whereas the field physical lines
    ...
    """A mapping from indices of lines (ordered as in ``lines``) to numerical tags for
    respective lines."""
...
class GmshData3d(_GmshData):
    ...
    Each item of the first  list is a numpy array of integers with the indices of
    ...
    edge must be reversed (relative to its description).
    ...
    """A map containing tags to identify the type of a polygon, per index of a polygon
    in ``polygons."""
...
class GmshWriter:
    ...
    constructed either by the method :meth:`generate`,
    or by directly working with the gmsh api in a client script.
    ...
    def __init__(self, data: GmshData2d | GmshData3d) -> None:
        ...
        Set during a call to :meth:`define_geometry`, which
