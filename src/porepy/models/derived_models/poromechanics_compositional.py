...
- Compositional flow (mass balance for H2O and NaCl components)
...
2. Porosity: \phi = \phi_ref + (\alpha-\phi_ref)(1-\phi)/K·Δp + \alpha·∇·u + thermal_contribution
...
class EquationsPoromechanicsCompositional(
    ...
    Equations: 
        - Momentum balance (displacement): ∇·σ + \rho_s.g = 0
...
class ConstitutiveLawsPoromechanicsCompositional(
    ...
    pp.constitutive_laws.ThermoPressureStress,  # \sigma_p = -\alpha·p·I, \sigma_T = -\beta·K·ΔT·I
    pp.constitutive_laws.ThermalExpansion,
    pp.constitutive_laws.ThermoPoroMechanicsPorosity,   # \phi(p, ∇·u, T)
    ...
    def stress(self, subdomains:list[pp.Grid]) -> pp.ad.Operator:
        ...
        \sigma = \sigma_mechanical - \alpha·p·I - \beta·K·ΔT·I
...
class SolutionStrategyPoromechanicsCompositional(
    ...
    def set_nonlinear_discretizations(self) -> None:
        ...
        subdomains_lower = [sd for sd in self.mdg.subdomains() if sd.dim < self.nd]
        if len(subdomains_lower) > 0:
            self.add_nonlinear_discretization(
                self.darcy_flux_discretization(subdomains_lower).flux(),
            )
