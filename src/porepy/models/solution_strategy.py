...
class SolutionStrategy(pp.PorePyModel):
    ...
    def __init__(self, params: Optional[dict] = None):
        ...
        ``model.params['reduce_linear_system'] == True``.
        
        - ``'row_perm_indices'`` indices/slice through the rows to obtain block diagonal
          form.
          The i-th element gives the original row index originally at row i.
        - ``'col_perm_indices'`` analogously for the columns.
        - ``'block_sizes'`` size of individual (square) blocks after the permutation.
          The sum of block sizes equals the dimension of the secondary block.

        Permutations generated and stored by :meth:`generate_block_permutation`.
    ...
    def schur_complement_inverter(self) -> Callable[[sps.spmatrix], sps.spmatrix]:
        ...
        The secondary block is defined by :meth:`secondary_equations` and
        :meth:`secondary_variables`, which are passed to the equation system in
        :meth:`assemble_linear_system`, if ``model.params['reduce_linear_system']`` is
        True.

        The base implementation assumes the secondary block to be a permuted, block
        ...
        See also:
            ...
            - :meth:`generate_block_permutation`
            - :meth:`invert_permuted_block_diag_mat`

        Note:
            This abstraction of assembling the inverter is intended for simulations
            where the DOFs might change from one time step to the other (f.e. fracture
            propagation).
            If the DOFs undergo any changes, the permutation generated by
            :meth:`generate_block_permutation` is invalid and must be computed again.
            This assembly must then be changed respectively.

        """

        def inverter(A: sps.csr_matrix) -> sps.csr_matrix:
            # Generate permutations only once, if not already present.
            if not self.secondary_block_permutation:
                self.generate_block_permutation(A)

            return self.invert_permuted_block_diag_mat(A)

        return inverter

    def generate_block_permutation(self, A: sps.csr_matrix) -> None:
        ...
        The permutation is stored in :attr:`secondary_block_permutation`.

        Notes:

            - The function assumes that the matrix is square or rectangular with
              consistent equation-variable relationships.
            - The permutations returned are such that applying them to the original
              matrix as ``A[row_perm, :][:, col_perm]`` would give a block-diagonal
              matrix.
        ...
        self.secondary_block_permutation = {
            "row_perm_indices": row_perm,
            "col_perm_indices": col_perm,
            "block_sizes": block_sizes,
        }

    def invert_permuted_block_diag_mat(
        self, A: sps.csr_matrix, eps: float = 1e-10
    ) -> sps.csr_matrix:
        """Compute A^{-1} by
        ...
        row_perm_idxs = self.secondary_block_permutation["row_perm_indices"]
        col_perm_idxs = self.secondary_block_permutation["col_perm_indices"]
        block_sizes = self.secondary_block_permutation["block_sizes"]
        row_slicer = pp.matrix_operations.ArraySlicer(domain_indices=row_perm_idxs)
        col_slicer = pp.matrix_operations.ArraySlicer(range_indices=col_perm_idxs)
