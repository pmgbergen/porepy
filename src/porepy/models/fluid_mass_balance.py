...
class InitialConditionsSinglePhaseFlow(pp.InitialConditionMixin):
    ...
    relative_well_flux: Callable[[list[pp.MortarGrid]], pp.ad.MixedDimensionalVariable]
    """See :class:`VariablesSinglePhaseFlow`."""
        ...
        for intf in self.mdg.interfaces():
            if intf.codim == 1:
                self.equation_system.set_variable_values(
                    ...
                    [cast(pp.ad.Variable, self.relative_interface_darcy_flux([intf]))],
                    iterate_index=0,
                )
            ...
            if intf.codim == 2:
                self.equation_system.set_variable_values(
                    ...
                    [cast(pp.ad.Variable, self.relative_well_flux([intf]))],
                    iterate_index=0,
                )
    ...
    def set_initial_values_primary_variables(self) -> None:
        ...
        for sd in self.mdg.subdomains():
            ...
            self.equation_system.set_variable_values(
                ...
                [cast(pp.ad.Variable, self.relative_pressure([sd]))],
                iterate_index=0,
            )
...
class VariablesSinglePhaseFlow(pp.VariableMixin):
    ...
    """
    has_single_phase_flow_reference_state: bool = False
    ...
    def reference_pressure(self, domains: pp.SubdomainsOrBoundaries) -> pp.ad.Operator:
        ...
        self.init_reference_single_phase_flow()
    ...
    def relative_pressure(self, domains: pp.SubdomainsOrBoundaries) -> pp.ad.Operator:
    ...
    def relative_interface_darcy_flux(
        self, interfaces: list[pp.MortarGrid]
    ) -> pp.ad.Operator:
    ...
    def relative_well_flux(self, interfaces: list[pp.MortarGrid]) -> pp.ad.Operator:
    ...
    def pressure(self, domains: pp.SubdomainsOrBoundaries) -> pp.ad.Operator:
        """Pressure term. Either a primary variable if subdomains are provided and
        ...
        """
        if len(domains) > 0 and isinstance(domains[0], pp.BoundaryGrid):
            return self.relative_pressure(domains)
        # Check that all domains are subdomains.
        if not all(isinstance(g, pp.Grid) for g in domains):
            raise ValueError("grids must consist entirely of subdomains.")
        # Now we can cast the grids
        domains = cast(list[pp.Grid], domains)
        return self.reference_pressure(domains) + self.relative_pressure(domains)
    ...
    def interface_darcy_flux(
        ...
        return self.reference_interface_darcy_flux(
            interfaces
        ) + self.relative_interface_darcy_flux(interfaces)
    ...
    def well_flux(
        ...
        """
        return self.reference_well_flux(interfaces) + self.relative_well_flux(
            interfaces
        )

    def init_reference_single_phase_flow(self) -> None:
        """Initialization of reference flow variables."""

        # NOTE: The reference state is needed in the creation of the variables,
        ...
        if self.has_single_phase_flow_reference_state:
            return
        self.has_single_phase_flow_reference_state = True
...
class DataSavingFluidMassBalance:
    ...
    def data_to_export(self):
        ...
        for sd in self.mdg.subdomains(return_data=False):
            ...
            # Delta pressure
            pressure_inc = self.evaluate_and_scale([sd], "relative_pressure", "Pa")
        ...
        for intf in self.mdg.interfaces(return_data=False, codim=1):
            ...
            flux_ref = self.evaluate_and_scale(
                [intf], "reference_interface_darcy_flux", "kg * m^-2 * s^-1"
            )
            data.append((intf, self.interface_darcy_flux_variable + "_ref", flux_ref))

            # Delta interface Darcy flux
            flux_inc = self.evaluate_and_scale(
                [intf], "relative_interface_darcy_flux", "kg * m^-2 * s^-1"
            )
            ...
            flux = self.evaluate_and_scale(
                [intf], "interface_darcy_flux", "kg * m^-2 * s^-1"
            )
        ...
        for intf in self.mdg.interfaces(return_data=False, codim=2):
            # Reference well flux
            well_ref = self.evaluate_and_scale(
                [intf], "reference_well_flux", "kg * m^-2 * s^-1"
            )
            data.append((intf, self.well_flux_variable + "_ref", well_ref))

            # Delta well flux
            well_inc = self.evaluate_and_scale(
                [intf], "relative_well_flux", "kg * m^-2 * s^-1"
            )
