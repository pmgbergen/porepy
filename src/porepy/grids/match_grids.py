...
The module is primarily intended used for replacing indivdiual grids in the GridTree,
which should be done via the GridTree method replace_grids(). That is, the methods
...
def gb_refinement(
    gb: pp.GridTree, gb_ref: pp.GridTree, tol: float = 1e-8, mode: str = "nested"
):
    ...
    assert len(grids) == len(
        grids_ref
    ), "Weakly check that GridTrees refer to same domains"
    assert np.allclose(
