...
class FvSubGrid(pp.Grid):
    ...
    def __init__(self, g, eta):
        ...
        if hasattr(g, "frac_pairs"):
            is_high = np.zeros(g.num_faces, dtype=np.bool)
            is_low = np.zeros(g.num_faces, dtype=np.bool)
            is_high[g.frac_pairs[0]] = True
            is_low[g.frac_pairs[1]] = True
            is_high_hf = is_high[subcell_topology.fno_unique]
            is_low_hf = is_low[subcell_topology.fno_unique]
            self.frac_pairs = np.vstack(
                (np.where(is_high_hf)[0], np.where(is_low_hf)[0])
            )
