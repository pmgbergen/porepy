...
def _generate_coarse_grid_gb(gb, subdiv):
    ...
    for g, (_, partition) in subdiv.items():
        ...
        # Update all the high_to_mortar_int for all the 'edges' connected to the grid
        # We update also all the face_cells
        for e, d in gb.edges_of_node(g):
            # The indices that need to be mapped to the new grid
            m2m = d["mortar_grid"].high_to_mortar_int().tocsr()
            indices = m2m.indices
            ...
            d["mortar_grid"]._high_to_mortar_int = new_m2m.tocsc()
...
def generate_seeds(gb):
    ...
    for g in gs:
        ...
        # recover the mapping between the low and the high grid
