...
class MaterialSubdomain:
    ...
    NOTE: It is unclear so far, how to proceed with the combination of Grids and substances (instead of GridBuckets)
    ...
    NOTE: We assume currently only a single substance. Mixed substance Domains remain a question for future development.
...
#------------------------------------------------------------------------------
    ...
    def porosity(self, law: str, *args, **kwargs) -> pp.ad.Operator:
        ...
        Currently supported heuristic laws (values for 'law'):
            - 'pressure':      expects one positional argument in 'args', namely the reference pressure
        ...
        if law == "pressure":
            ...
            return pp.ad.Function(lambda p: self.substance.base_porosity()*(p - p_ref),
            "porosity-%s-%s"%(law, self.substance.name))(pressure)
        else:
            raise NotImplementedError("Unknown 'law' keyword for subdomain porosity.: %s \n"%(law)
            + "Available: 'pressure,'")
