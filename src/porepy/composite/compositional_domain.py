...
from porepy.composite.substance import Substance, SolidSubstance
from porepy.composite.phase import PhaseField
...
class CompositionalDomain:
    ...
    Constructs computational elements for the simulation, by
        - combining the geometrical functions provided by a
          :class:`~porepy.grids.grid_bucket.GridBucket`
        - providing calls to global, physical variables
          (merged variables from the AD submodule)
    ...
    def __call__(
        self, variable_name: str, dof_info: Optional[Dict[str, int]] = {"cells": 1}
    ) -> pp.ad.MergedVariable:
        """Returns a reference to the unique, domain wide variable with a distinguishable name.
        Accessing the same variable is of importance in inherited models and multi-physics
        scenarios.
        ...
        :param variable_name: symbolic variable name
        (see :data:`~porepy.params.computational_variables.COMPUTATIONAL_VARIABLES`)
        :type variable_name: str
        :param dof_info: (optional) number of DOFs per grid element (e.g. cells, faces, nodes).
        Defaults to 1 DOF per cell
        :type dof_info: dict
        ...
        if variable_name in self._global_ad.keys():
            # TODO case when variable exists AND new DOF information is given
            # (dereference old var and create new one)
            ...
            if COMPUTATIONAL_VARIABLES["mortar_prefix"] == split_name[0]:
                # NOTE VL: Think about introducing some kind of validation for the symbol,
                # to avoid arbitrary creation of variables
                # symbol = split_name[1]
                is_mortar = True
    ...
    def assign_material_to_grid(self, grid: pp.Grid, substance: SolidSubstance) -> None:
        ...
        You can use the iterator of this instances
    ...
    def add_phase(self, phases: Union[List[PhaseField], PhaseField]) -> None:
        ...
        # if it's name is already among the present phases
        for phase in phases:
            if phase.cd != self:
                raise ValueError(
                    "Phase '%s' instantiated on unknown ComputationalDomain."
                    % (phase.name)
                )
    ...
    def set_initial_values(
        ...
        THE FOLLOWING IS ASSUMED FOR THE ARGUMENTS:
            ...
            - the order of this instances iterator
              :method:`~porepy.composite.computational_domain.Phases` is assumed for the values
              in the nested lists
    ...
    def resolve_composition(self) -> None:
        ...
        This method is called internally by phases and domains,
        everytime any new component is added.
        """
    ...
    def compute_initial_equilibrium(self) -> None:
        ...
        Use :method:`~porepy.compostie.phase.PhaseField.set_initial_fractions_in_phase`
        to set initial molar fractions per phase
        Use
        :method:`~porepy.compostie.compositional_domain.CompositionalDomain.set_initial_values`
        to set the rest.
        ...
        The reason for splitting it between the PhaseField class and this class is for reasons
        of easier overview, and to give more responsibility to the PhaseField.
        """
    ...
    def _set_initial_overall_molar_fraction(
        self, initial_values: List["np.ndarray"]
    ) -> None:
        ...
        Keep in mind that the sum over all components  has to be 1 at all times.
        ...
        for grid_data, val in zip(self.gb, initial_values):
            ...
            # create a copy to avoid issues by multiple references
