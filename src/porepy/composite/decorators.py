...
namely at that point when the decorated object gets imported into the runtime, i.e. the code is interpreted.

Python expects the decorator class to be callable.
A call using the decorated method as an argument is done immediately after instantiation of the decorator class.
"""
...
class THMSignature:
    ...
    Physical variables are defined in :data:`~porepy.composite._composite_utils.COMPUTATIONAL_VARIABLES` (keys).

    EXAMPLE: for a instance bound method calculating the density of carbonized brine:
    .. code-block:: python
        @THMSignature("pressure", "temperature", "component_fraction_in_phase", "component_fraction_in_phase")
        def density(self, p: float, T: float, chi_NaCl: float, chi_CO2: float) -> float:
            return ...

    NOTE: As of now, only (global) pressure and temperature are supported as thermodynamic state variables in the composite submodule
    It is intended that this class converts information properly in future. This needs some more work.
    ...
    def __init__(self, *args) -> None:
        ...
        Throws an error if the physical signature argument is not known to :data:`~porepy.composite._composite_utils.COMPUTATIONAL_VARIABLES`.
        
        :param args: iterable of strings, where the strings represent thermal-hydraulic-mechanic physical quantities e.g., pressure, displacement,...
        :type args: Tuple[str]
        """
    ...
    def __call__(self, method: FunctionType) -> Any:
        ...
        IMPORTANT: The 'FunctionType' argument 'method' is expected to have a descriptor protocol implemented.
        ...
        # 'wraps' assures that the wrapper has the same magic attributes (meta-information) as the actual method (such as __name__ and __doc__)
        # it also changes the signature, so that the class instance, whose method is being wrapped, appears as a separate, first argument
        @wraps(method)
...
class ADProperty:
    ...
    The Jacobian of the blackbox function is approximated according to the AD type passed at instantiation.
    ...
    def __init__(self, ad_type: TypeVar("ApproximateJacobianFunction-type", pp.ad.ApproximateJacobianFunction)) -> None:
        ...
        :type ad_ype: TypeVar[:class:`~porepy.numerics.ad.operators.ApproximateJacobianFunction`]
        """
    ...
    def __call__(self, method: FunctionType) -> FunctionType:
        ...
        IMPORTANT: The 'FunctionType' argument 'method' is expected to have a descriptor protocol implemented.
        I.e. when called, it is supposed to return a bounded method (class method f.e.).
        This means, the first argument of 'method' is supposed to be an instance method of a class.
        ...
        wrapper.__doc__ += "\nDecorated AD-attribute using 'pp.ad.%s'"%(str(self._ad_type.__name__))
