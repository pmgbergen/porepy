...
class PengRobinsonEoS(AbstractEoS):
    ...
    def compressibility_factor(
        ...
        B: sm.Expr,
        eps: float = 1e-14,
        smooth_3root: float = 1e-2,
        smooth_ext: float = 1e-1,
    ) -> tuple[sm.Expr, sm.Expr]:
        """Symbolic computation of compressibility factor.

        Parameters:
            A: Non-dimensional cohesion for phase (mixed).
            B: Non-dimensional covolume for phase (mixed).
            eps: ``default=1e-14``

                Numerical zero.
            smooth_3root: ``default=1e-2``

                Small number to define region of subcritical 2-phase region for
                smoothing.
            smooth_ext: ``default=1e-1``

                Small number to define region around borders between different
                representations of extended roots for smoothing.

        Returns:
            A 2-tuple containing the liquid- and gas-like root in this order.
        ...
        r = c1 - c2**2 / 3
        q = 2 / 27 * c2**3 - c2 * c1 / 3 + c0

        # discriminant to determine the number of roots
        delta = q**2 / 4 + r**3 / 27

        # degenerate polynomial if discriminant is zero:
        # one double root or a triple root
        degenerate_case = (delta > -eps) & (delta < eps)

        # Different root cases
        double_root_case = degenerate_case & ((r <= -eps) | (r >= eps))
        triple_root_case = degenerate_case & ((r > -eps) & (r < eps))
        one_root_case = delta > eps
        three_root_case = delta < -eps

        # At A,B=0 we have 2 real roots, one with multiplicity 2
        zero_point = (
            (A > -eps) & (A < eps) & (B > -eps) & (B < eps)
        )
        double_root_case = double_root_case | zero_point
        ...
        triple_root_case = triple_root_case | critical_point

        # identify super-critical line
        is_supercritical = B >= cls.critical_line(A)
        # check if below Widom line TODO: Data for water is used here.
        below_widom = B <= cls.Widom_line(A)

        # subcritical area for Ben Gharbia extension
        gharbia_ext = (~is_supercritical) & (B < cls.B_CRIT)
        # Supercritical areas for asymmetric extensions
        liq_ext_supc = is_supercritical & (~below_widom)
        gas_ext_supc = (~gharbia_ext) & below_widom

        # The following functions implement the Cardano formulae for polynomials of 3rd
        # order. Taken from: https://de.wikipedia.org/wiki/Cardanische_Formeln
        def triple_root(c2_) -> tuple[sm.Expr, sm.Expr]:
            return - c2_ / 3 , -c2_ / 3

        def double_root(c2_, r_, q_) -> tuple[sm.Expr, sm.Expr]:
            ...
            zg_ = sm.Piecewise((z1_, z1_ > z2_), (z2_, True))
            zl_ = sm.Piecewise((z2_, z1_ > z2_), (z1_, True))

            return zl_, zg_
        
        def three_root(c2_, r_, q_) -> tuple[sm.Expr, sm.Expr]:

            # trigonometric formula for Casus Irreducibilis
            t_2 = sm.acos(-q_ / 2 * sm.sqrt(-27 * r_ **(-3))) / 3
            t_1 = sm.sqrt(-4 / 3 * r_)

            z3 = t_1 * sm.cos(t_2) - c2_ / 3
            z2 = -t_1 * sm.cos(t_2 + np.pi / 3) - c2_ / 3
            z1 = -t_1 * sm.cos(t_2 - np.pi / 3) - c2_ / 3

            # smoothing roots
            p = (z2 - z1) / (z3 - z1) # proximity of intermediate root

            v_g_ = (p - (1 - 2 * smooth_3root)) / smooth_3root
            v_g_ = v_g_ ** 2 * (3 - 2 * v_g_)  # gas root smoothing weight

            v_g = sm.Piecewise(
                (1, p >= 1 - smooth_3root ),
                (0, p <= 1 - 2 * smooth_3root),
                (v_g_, True),
            )
            w_g = (z2 + z3) / 2  # smoothed gas root

            v_l_ = (p - smooth_3root) / smooth_3root
            v_l_ = -1 * v_l_**2 * (3 - 2 * v_l_) + 1
            ...
            z1_s = z1 * (1 - v_l) + w_l * v_l
            z3_s = z3 * (1 - v_g) + w_g * v_g


            z_l = sm.Piecewise(
                (z1_s, gharbia_ext),  # use smoothing only where 3 roots are physical
                (cls.extended_root_liquid_sc(B, z3), True),  # use extension in 3-root region above critical line
            )
            z_g = sm.Piecewise(
                (z3_s, gharbia_ext),
                (z3, True)
        ...
        def one_root(c2_, r_, q_, delta_) -> tuple[sm.Expr, sm.Expr]:
            
            t = -q_ / 2 + sm.sqrt(delta_)
            u = sm.Piecewise((-1 * sm.cbrt(-1 * t), t < 0), (sm.cbrt(t), True))

            z = u - r_ / (u * 3) - c2_ / 3

            w_sub = cls.extended_root_sub(B, z)
            w_sc_l = cls.extended_root_liquid_sc(B, z)
            w_sc_g = cls.extended_root_gas_sc(B, z)

            # Smoothing if point A-B is close to borders between different extension
            # procedures
            ab = [A, B]
            widom_points = np.array(
                [[0, cls.Widom_line(0)], [cls.A_CRIT, cls.Widom_line(cls.A_CRIT)]]
            )
            B_crit_points= np.array(
                [[0, cls.B_CRIT], [cls.A_CRIT, cls.B_CRIT]]
            )
            critline_points = np.array(
                [[0, 0], [cls.A_CRIT, cls.B_CRIT]]
            )

            # distance to Widom line
            d_w = _point_to_line_distance_symb(ab, widom_points)
            # smoothing between different supercrit extensions
            smooth_w = (d_w <= smooth_ext) & (B >= cls.B_CRIT) & (~ below_widom)
            d_w_n = d_w / smooth_ext
            w_w = w_sc_g * (1 - d_w_n) + w_sc_l * d_w_n

            # distance to supercrit line
            d_s = _point_to_line_distance(ab, critline_points)
            # Smoothing between extensions above and below supercrit line
            smooth_s = (d_s <= smooth_ext) & (B < cls.B_CRIT) & is_supercritical
            d_s_n = d_s / smooth_ext
            w_s = w_sub * (1 - d_s_n) + w_sc_l * d_s_n

            # distance to line B=B_crit
            d_b = _point_to_line_distance_symb(ab, B_crit_points)
            # Smoothing between extensions above and below line B=B_crit
            smooth_b = (d_b <= smooth_ext) & (B >= cls.B_CRIT) & below_widom
            d_b_n = d_b / smooth_ext
            w_b = w_sub * (1 - d_b_n) + w_sc_g * d_b_n
            ...
            z_l = sm.Piecewise((w, ~ below_widom), (z, True))
            z_g = sm.Piecewise((w, below_widom), (z, True))

            return z_l, z_g

        Z_t = triple_root(c2)
        Z_d = double_root(c2, r, q)
        Z_3 = three_root(c2, r, q)
        Z_1 = one_root(c2, r, q, delta)

        Z_L = sm.Piecewise(
            (Z_1[0], one_root_case),
            (Z_3[0], three_root_case),
            (Z_t[0], three_root_case),
            (Z_d[0], double_root_case)
        )
        Z_G = sm.Piecewise(
            (Z_1[1], one_root_case),
            (Z_3[1], three_root_case),
            (Z_t[1], three_root_case),
            (Z_d[1], double_root_case)
        )
