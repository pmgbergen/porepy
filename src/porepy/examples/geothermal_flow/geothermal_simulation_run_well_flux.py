...
from porepy.examples.geothermal_flow.model_configuration.flow_model_config_well_flux import (
...
from porepy.examples.geothermal_flow.model_configuration.geometry_description.geometry_market import DomainConnectedFracture2D as ModelGeometry2D
...
SIMULATION_CASES = {
    "three_phase_LP_2D": {  # Low-pressure two-phase (Figure 4)
        "BC": BCLiquidPhaseLowPressure_Well_Flux,
        "IC": ICLiquidPhaseLowPressure_Well_Flux,
...
def create_dynamic_model(BC, IC, FlowModel):
    ...
    class GeothermalSimulationFlowModel(ModelGeometry2D, BC, IC, FlowModel):
        ...
        def after_nonlinear_convergence(self):
            ...
            # s_h_new = halite_phase.saturation(subdomains).value(self.equation_system)
            # # Store for next Newton iteration
            # self._s_halite_prev = s_h_new.copy()
