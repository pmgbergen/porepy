...
class SimpleGeometryHorizontal(Geometry):
    ...
    _dist_from_ref_point: float = 2.0
    _inlet_centre: np.ndarray = np.array([0.0, 2.0, 0.0])
    _outlet_centre: np.ndarray = np.array([2000.0, 2.0, 0.0])
    ...
    def meshing_arguments(self) -> dict:
        cell_size = self.units.convert_units(4.0, "m")
        mesh_args: dict[str, float] = {"cell_size": cell_size}
        return mesh_args
...
class SimpleGeometryVertical(Geometry):
    ...
    _outlet_centre: np.ndarray = np.array([2.0, 2000.0, 0.0])
