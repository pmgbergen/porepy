...
class SimpleGeometryHorizontal_Old(Geometry):
...
class SimpleGeometryHorizontal(Geometry):
    ...
    def get_inlet_outlet_sides(
        ...
        def find_facets(is_left: bool) -> np.ndarray:
            domain_sides = self.domain_boundary_sides(sd)
            if is_left:
                return np.where(domain_sides.west==True)[0]
            else:
                return np.where(domain_sides.east==True)[0]
            
        inlet_facets = find_facets(is_left=True)
        outlet_facets = find_facets(is_left=False)
        return inlet_facets, outlet_facets
...
class DomainFractureWellLeftDirichlet2D(Geometry):
    ...
    def meshing_arguments(self) -> dict:
        cell_size = self.units.convert_units(0.7, "m") # was 0.3
        frac_cell_size = self.units.convert_units(1.0, "m") # was 0.5
        mesh_args: dict[str, float] = {
            "cell_size": cell_size,
            "cell_size_fracture": frac_cell_size
        }
    ...
    #     # --- SNAP WELL TO CELL CENTER (2D -> 3D) ---
    #     # Find closest cell *by center* and then place the well exactly at that center
    #     cell_idx = int(matrix.closest_cell(point.reshape(2, 1))[0])
    #     snapped_xy = matrix.cell_centers[:2, cell_idx]   # (2,)
    #     p = np.zeros(3)
    #     p[:2] = snapped_xy

    #     # Create 0D grid at snapped location
    #     sd_0d = pp.PointGrid(p)
    #     sd_0d.tags[f"{well_type}_well"] = well_index
    #     sd_0d.compute_geometry()
    #     self.mdg.add_subdomains(sd_0d)

    #     # Couple well to the same cell (now deterministic)
    #     cell_well = np.array([0], dtype=int)
    #     cell_matrix = np.array([cell_idx], dtype=int)

    #     cell_cell_map = sps.coo_matrix(
    #         (np.ones(1, dtype=bool), (cell_well, cell_matrix)),
    #         shape=(sd_0d.num_cells, matrix.num_cells),
    #     )
    #     print(f"{well_type} well requested at {point}, snapped to {snapped_xy}, cell {cell_idx}")
    ...
    def get_inlet_outlet_sides(self, sd: pp.Grid | pp.BoundaryGrid) -> tuple[np.ndarray, np.ndarray]:
        ...
        domain_sides = self.domain_boundary_sides(sd)
        left_bc = np.where(domain_sides.west==True)[0]
        right_bc = np.where(domain_sides.west==True)[0]
...
class DomainConnectedFracture2DNew(Geometry):
    """A class to represent a simple 2D geometry for a simulation domain. 
...
class DomainFractureWellLeftDirichlet2D_ManyFracture(Geometry):
    """A class to represent a simple 2D geometry for a simulation domain. 
