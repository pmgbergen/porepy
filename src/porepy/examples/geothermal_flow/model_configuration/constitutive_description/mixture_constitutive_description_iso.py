...
def enthaply_from_pressure(
    self,
    p: np.ndarray,
    z: np.ndarray = None,
) -> np.ndarray:
    """Convert temperature [K] to specific enthalpy [J/kg] at varying pressure [Pa].
        Assumes pure water.
    """
    # constant temperature
    n_cell = len(p)
    T = 673.15 * np.ones(n_cell)  # 150°C in Kelvin
    if z is None:
...
class LiquidDriesnerCorrelations(pp.compositional.EquationOfState):
    ...
    def vtk_sampler_ptz(self, vtk_sampler_ptz: VTKSampler):
        self._vtk_sampler_ptz = vtk_sampler_ptz
    ...
    def compute_phase_properties(
        ...
        *thermodynamic_input: np.ndarray,
        ...
        if len(thermodynamic_input) == 3:
            ...
            p, z_NaCl = thermodynamic_input
            h = enthaply_from_pressure(self, p, z_NaCl)
        elif len(thermodynamic_input) < 2:
            p = thermodynamic_input[0]
            z_NaCl = np.zeros_like(p)
            h = enthaply_from_pressure(self, p)
...
class FluidMixture(pp.PorePyModel):
    ...
    def get_phase_configuration(
        ...
        for state in active_phases:
            ...
            eos.vtk_sampler_ptz = self.vtk_sampler_ptz
    ...
    def dependencies_of_phase_properties(
        ...
        return [self.pressure] + z_NaCl  # type:ignore[return-value]
...
class SecondaryEquations(LocalElimination):
    ...
    # ) -> tuple[np.ndarray, np.ndarray]:
    #     """Constant temperature."""
    #     p = thermodynamic_dependencies[0]
    #     n = len(p)
        
    #     T = 623.15 * np.ones(n)  # 150°C in Kelvin
    #     dT = np.zeros((len(thermodynamic_dependencies), n))  # All zeros
    ...
    def gas_saturation_func(
        ...
        *thermodynamic_dependencies: np.ndarray,
        ...
        if len(thermodynamic_dependencies) == 3:
            ...
            p, z_NaCl = thermodynamic_dependencies
            h = enthaply_from_pressure(self, p, z_NaCl)
        elif len(thermodynamic_dependencies) < 2:
            p = thermodynamic_dependencies[0]
            z_NaCl = np.zeros_like(p)
        ...
        dS_v = [dS_vdp]
        if len(thermodynamic_dependencies) == 2:
            dS_v.append(dS_vdz)
        if len(thermodynamic_dependencies) == 3:
            dS_v.append(dS_vdH)
            dS_v.append(dS_vdz)
    ...
    def H2O_liq_func(
        ...
        *thermodynamic_dependencies: np.ndarray,
        ...
        if len(thermodynamic_dependencies) == 2:
            dX_w.append(dX_wdz)
        elif len(thermodynamic_dependencies) == 3:
            dX_w.append(dX_wdH)
            dX_w.append(dX_wdz)
        dX_w = np.vstack(dX_w)
        return X_w, dX_w
    ...
    def set_equations(self) -> None:
        ...
        # Temperature: always needed since we solve energy equation.
        # ref_phase = self.fluid.reference_phase
        # self.eliminate_locally(
        #     self.temperature,
