...
class FluidMixture(ppc.FluidMixtureMixin):
    ...
    def get_phase_configuration(
        self, 
        components: Sequence[ppc.Component]
...
class SecondaryEquations(SecondaryEquationsMixin):
    ...
    def NaCl_liq_func(
        ...
        *thermodynamic_dependencies: np.ndarray,
        ...
        X_s = 0.0 * self.vtk_sampler.sampled_could.point_data["Xl"]
        dX_sdz = 0.0 * self.vtk_sampler.sampled_could.point_data["grad_Xl"][:, 0]
        dX_sdH = 0.0 * self.vtk_sampler.sampled_could.point_data["grad_Xl"][:, 1]
        dX_sdp = 0.0 * self.vtk_sampler.sampled_could.point_data["grad_Xl"][:, 2]
    ...
    def NaCl_gas_func(
        ...
        *thermodynamic_dependencies: np.ndarray,
        ...
        X_s = 0.0 * self.vtk_sampler.sampled_could.point_data["Xv"]
        dX_sdz = 0.0 * self.vtk_sampler.sampled_could.point_data["grad_Xv"][:, 0]
        dX_sdH = 0.0 * self.vtk_sampler.sampled_could.point_data["grad_Xv"][:, 1]
        dX_sdp = 0.0 * self.vtk_sampler.sampled_could.point_data["grad_Xv"][:, 2]
        dX_s = np.vstack((dX_sdp, dX_sdH, dX_sdz))
        return X_s, dX_s
