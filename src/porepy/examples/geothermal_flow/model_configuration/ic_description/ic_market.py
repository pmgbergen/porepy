...
class ICThreePhaseLowPressure(pp.PorePyModel):
    ...
    def ic_values_pressure(self, sd: pp.Grid) -> np.ndarray:
        ...
        p_inlet = 4.0e6
        p_outlet = 1.0e6
        domain_length = 2000.0  # or get from self.domain.bounding_box
        cell_centers_x = sd.cell_centers[0]
        pressure_gradient = (p_outlet - p_inlet) / domain_length
        p_init = p_inlet + pressure_gradient * cell_centers_x
        return p_init
...
class ICLiquidPhaseLowPressure_Pointwells_Heter_Fracture_Salt_2D(pp.PorePyModel):
    ...
    def ic_values_pressure_old(self, sd: pp.Grid) -> np.ndarray:
        """Define initial pressure distribution.
        - Matrix: linear gradient from injector (left) to producer (right).
        - Fractures / wells: inherit pressure from the matrix cells they sit in.
        """

        # Case 1: production wells
        if sd.dim == 0 and "production_well" in sd.tags:
            return np.ones(sd.num_cells) * self._p_PRODUCTION[0]

        # Case 2: matrix grid (full 2D domain)
        if sd.dim == self.domain.dim:
            # x = np.where(sd.cell_centers[0, :] >= 15)[0]
            x = sd.cell_centers[0, :]
            x_min, x_max = x.min(), x.max()

            p_left = 12.0E6  # [Pa] # self._p_INIT[0]
            p_right = 9.0E6  # [Pa] # self._p_PRODUCTION[0]

            p = p_left + (p_right - p_left) * ((x - x_min) / (x_max - x_min))
            return p

        # Case 3: fracture or lower-dim subdomain
        if sd.dim < self.domain.dim:
            # get reference to the matrix grid
            matrix = self.mdg.subdomains(dim=self.domain.dim)[0]

            # map fracture cell centers to closest matrix cell
            cell_matrix = matrix.closest_cell(sd.cell_centers)
            p_matrix = self.ic_values_pressure(matrix)

            return p_matrix[cell_matrix]
    ...
    def ic_values_enthalpy(self, sd: pp.Grid) -> np.ndarray:
        ...
        t = self.ic_values_temperature(sd)
        z_NaCl = self._z_INIT["NaCl"] * np.ones_like(p)
        # T_values = find_T_for_target_s_h_l(
        #     z0=0.2,
        #     h_target=2.350e6, 
        #     p_values=np.linspace(8.0e6, 30.0e6, 50),  # p[0:1], 
        #     sampler=self.vtk_sampler_ptz,
        #     T_bounds=(473.15, 1000.15), #(473.15, 543.15), works with 20 bar
        #     T_size=10
        # )
        # T_values = find_T_for_target_s_h_l_v(
        #     z0=0.2,
        #     h_target=2.350e6, 
        #     p_values=np.array([2.0e6]),  # p[0:1], 
        #     sampler=self.vtk_sampler_ptz,
        #     T_bounds=(473.15, 473.15),
        #     T_size=10
        # )
        assert len(p) == len(t) == len(z_NaCl)
        par_points = np.array((z_NaCl, t, p)).T
...
class ICLiquidPhaseLowPressure_Well_Flux(pp.PorePyModel):
    ...
    def ic_values_pressure(self, sd: pp.Grid) -> np.ndarray:
        ...
        # return np.ones(sd.num_cells) * self._p_INIT
        if sd.dim == 0 and "production_well" in sd.tags:
            return np.ones(sd.num_cells) * self.adaptive_production_pressure()  # self._p_PRODUCTION[0]
        # if sd.dim == 0 and "injection_well" in sd.tags:
        #     return np.ones(sd.num_cells) * self._p_INJECTION[0]  
        return np.ones(sd.num_cells) * self._p_INIT
    ...
    def ic_values_pressure_old(self, sd: pp.Grid) -> np.ndarray:
        ...
        if sd.dim == self.domain.dim:
            ...
            p_left = self._p_INIT
            p_right = 8.5E6  # [Pa] # self._p_PRODUCTION[0]
    ...
    def ic_values_enthalpy(self, sd: pp.Grid) -> np.ndarray:
        ...
        if "injection_well" in sd.tags:
            z_NaCl = np.ones(sd.num_cells) * self._z_INJ.get("NaCl", 0.0)
        else:
            z_NaCl = self._z_INIT["NaCl"] * np.ones_like(p)
        # T_values = find_T_for_target_s_h_l(
        #     z0=0.2,
...
class ICLiquidPhaseLowPressure_SmallBox(pp.PorePyModel):
    ...
    def ic_values_pressure_old(self, sd: pp.Grid) -> np.ndarray:
        ...
        x = sd.cell_centers[0, :]   # shape (num_cells,)
        y = sd.cell_centers[1, :]   # shape (num_cells,)

        # producer location
        x_w, y_w = self._production_points[0]
        ...
        p_left, p_prod = self._p_INJECTION, self._p_PRODUCTION[0]
        
        # linear radial decline
        p = p_prod + (p_left - p_prod) * (r / R)
        return p
...
class ICLiquidPhaseLowPressure_Well_Flux_Dir_Left_old(pp.PorePyModel):
...
class ICLiquidPhaseLowPressure_Well_Flux_Dir_Left(pp.PorePyModel):
    ...
    def ic_values_pressure(self, sd: pp.Grid) -> np.ndarray:
        ...
        # return np.ones(sd.num_cells) * self._p_INIT
        if sd.dim == 0 and "production_well" in sd.tags:
            return np.ones(sd.num_cells) * self.get_ramped_production_bhp()
        return np.ones(sd.num_cells) * self._p_INIT
    ...
    def ic_values_enthalpy(self, sd: pp.Grid) -> np.ndarray:
        ...
        t = self.ic_values_temperature(sd)
        if "injection_well" in sd.tags:
            z_NaCl = np.ones(sd.num_cells) * self.get_ramped_injection_composition("NaCl")
        else:
            z_NaCl = self._z_INIT["NaCl"] * np.ones_like(p)
        # z_NaCl = self._z_INIT["NaCl"] * np.ones_like(p)
        assert len(p) == len(t) == len(z_NaCl)
    ...
    def ic_values_temperature(self, sd: pp.Grid) -> np.ndarray:
        # return np.ones(sd.num_cells) * self._T_INIT
        if sd.dim == 0 and "injection_well" in sd.tags:
            return np.ones(sd.num_cells)*self.get_ramped_injection_temperature()
        return np.ones(sd.num_cells) * self._T_INIT
    ...
    def ic_values_overall_fraction(
        ...
        """Initial composition: default to initial z."""
        if sd.dim == 0 and "injection_well" in sd.tags:
            return np.ones(sd.num_cells)*self.get_ramped_injection_composition(
                component.name
            )
        z = self._z_INIT.get(component.name, 0.0)
        return np.full(sd.num_cells, z)
