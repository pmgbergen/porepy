...
def x_range_predicate(x, y, z, bounds):
    xmin, xmax, ymin, ymax, zmin, zmax = bounds
    x_range_pre = np.logical_and(np.logical_or(x > xmin, np.isclose(x, xmin)),
                                 np.logical_or(x < xmax, np.isclose(x, xmax)))
    return x_range_pre

def y_range_predicate(x, y, z, bounds):
    xmin, xmax, ymin, ymax, zmin, zmax = bounds
    y_range_pre = np.logical_and(np.logical_or(y > ymin, np.isclose(y, ymin)),
                                 np.logical_or(y < ymax, np.isclose(y, ymax)))
    return y_range_pre

def z_range_predicate(x, y, z, bounds):
    xmin, xmax, ymin, ymax, zmin, zmax = bounds
    z_range_pre = np.logical_and(np.logical_or(z > zmin, np.isclose(z, zmin)),
