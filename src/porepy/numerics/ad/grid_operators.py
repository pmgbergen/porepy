    ...
    def __init__(
        ...
        for g in grids:
            if self._is_scalar:
                ...
                div_extended = sps.kron(div, eye_nd)
                div_cell = div_extended.T * cell_projections[g].T
                div_face = div_extended * face_projections[g].T
                
                # Values to return
                trace.append(div_cell) 
                inv_trace.append(div_face)
            # end if-else
