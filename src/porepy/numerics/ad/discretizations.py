"""
For any discretization class compatible with PorePy, wrap_discretization associates
a discretization with all attributes of the class' attributes that end with
'_matrix_key'.


Example:
    # Generate grid
    >>> g = pp.CartGrid([2, 2])
    # Associate an Ad representation of an Mpfa method, aimed this grid
    >>> discr = MpfaAd(keyword='flow', grids=[g])
    # The flux discretization of Mpfa can now be accesed by
    >>> discr.flux
    # While the discretization of boundary conditions is available by
    >>> discr.bound_flux.

    The representation of different discretization objects can be combined with other
    Ad objects into an operator tree, using lazy evaluation.

    It is assumed that the actual action of discretization (creation of the
    discretization matrices) is performed before the operator tree is parsed.
"""

from __future__ import annotations

import abc
from typing import TYPE_CHECKING, Callable, Optional

import porepy as pp

if TYPE_CHECKING:
    from porepy.utils.porepy_types import discretization_type

__all__ = [
    "DiscretizationAd",
    "BiotAd",
    "MpsaAd",
    "TpsaAd",
    "MpfaAd",
    "TpfaAd",
    "UpwindAd",
    "UpwindCouplingAd",
]


class DiscretizationAd(abc.ABC):
    """General/utility methods for AD discretization classes.

    The init of the children classes below typically calls wrap_discretization
    and has arguments including subdomains or interfaces and keywords for parameter and
    possibly matrix storage.

    """

    def __init__(self) -> None:
        self._discretization: discretization_type
        """The discretization object, which is wrapped by this class."""
        self.mat_dict_key: str
        """Keyword for matrix storage in the data dictionary."""
        self.keyword: str
        """Keyword for parameter storage in the data dictionary."""

        self.subdomains: list[pp.Grid]
        """List of grids on which the discretization is defined."""
        self.interfaces: list[pp.MortarGrid]
        """List of interfaces on which the discretization is defined."""

        self._name: str

    def __repr__(self) -> str:
        s = f"""
        Ad discretization of type {self._name}. Defined on {len(self.subdomains)}
        subdomains and {len(self.interfaces)} interfaces.
        """
        return s

    def __str__(self) -> str:
        return f"{self._name}({self.keyword})"

    @property
    def name(self) -> str:
        """Name of the discretization."""
        return self._name


# Mechanics related discretizations


class BiotAd(DiscretizationAd):
    """Ad wrapper around the Biot discretization class.

    For description of the method, we refer to the standard Biot class.

    """

    def __init__(self, keyword: str, subdomains: list[pp.Grid], nd: int) -> None:
        self.subdomains = subdomains
        self._discretization = pp.Biot(keyword)
        self._name = "BiotMpsa"

        self.keyword = keyword

        # Declare attributes, these will be initialized by the below call to the
        # discretization wrapper.

        self.stress: Callable[[], pp.ad.MergedOperator]
        self.bound_stress: Callable[[], pp.ad.MergedOperator]
        self.bound_displacement_cell: Callable[[], pp.ad.MergedOperator]
        self.bound_displacement_face: Callable[[], pp.ad.MergedOperator]

        # Attributes for the coupling terms
        self.displacement_divergence: Callable[[str], pp.ad.MergedOperator]
        self.bound_displacement_divergence: Callable[[str], pp.ad.MergedOperator]
        self.scalar_gradient: Callable[[str], pp.ad.MergedOperator]
        self.consistency: Callable[[str], pp.ad.MergedOperator]
        self.bound_pressure: Callable[[str], pp.ad.MergedOperator]

        # The following are keywords used to identify the coupling terms constructed
        # by a Biot discretization.
        coupling_terms = [
            "displacement_divergence",
            "bound_displacement_divergence",
            "scalar_gradient",
            "bound_pressure",
            "consistency",
        ]

        pp.ad.wrap_discretization(
            obj=self,
            discr=self._discretization,
            nd=nd,
            subdomains=subdomains,
            coupling_terms=coupling_terms,
        )


class MpsaAd(DiscretizationAd):
    def __init__(self, keyword: str, subdomains: list[pp.Grid], nd: int) -> None:
        self.subdomains = subdomains
        self._discretization = pp.Mpsa(keyword)
        self._name = "Mpsa"

        self.keyword = keyword

        # Declare attributes, these will be initialized by the below call to the
        # discretization wrapper.

        self.stress: Callable[[], pp.ad.MergedOperator]
        self.bound_stress: Callable[[], pp.ad.MergedOperator]
        self.bound_displacement_cell: Callable[[], pp.ad.MergedOperator]
        self.bound_displacement_face: Callable[[], pp.ad.MergedOperator]

        pp.ad.wrap_discretization(
            self, self._discretization, nd=nd, subdomains=subdomains
        )


class TpsaAd(DiscretizationAd):
    def __init__(self, keyword: str, subdomains: list[pp.Grid], nd: int) -> None:
        self.subdomains = subdomains
        self._discretization = pp.Tpsa(keyword)
        self._name = "Tpsa"

        self.keyword = keyword

        # Declare attributes, these will be initialized by the below call to the
        # discretization wrapper.
        self.stress_displacement: Callable[[], pp.ad.MergedOperator]
        """Stress generated by displacement degrees of freedom."""
        self.bound_stress: Callable[[], pp.ad.MergedOperator]
        """Stress generated by mechanics boundary conditions."""
        self.stress_rotation: Callable[[], pp.ad.MergedOperator]
        """Stress generated by rotation degrees of freedom."""
        self.stress_total_pressure: Callable[[], pp.ad.MergedOperator]
        """Stress generated by total pressure degrees of freedom."""

        self.rotation_displacement: Callable[[], pp.ad.MergedOperator]
        """Rotation generated by displacement degrees of freedom."""
        self.rotation_rotation: Callable[[], pp.ad.MergedOperator]
        """Rotation generated by rotation degrees of freedom."""
        self.bound_rotation_displacement: Callable[[], pp.ad.MergedOperator]
        """Rotation generated by mechanics boundary conditions."""

        self.mass_total_pressure: Callable[[], pp.ad.MergedOperator]
        """Solid mass flux generated by total pressure degrees of freedom."""
        self.mass_displacement: Callable[[], pp.ad.MergedOperator]
        """Solid mass flux generated by displacement degrees of freedom."""
        self.bound_mass_displacement: Callable[[], pp.ad.MergedOperator]
        """Solid mass flux generated by mechanics boundary conditions."""

        pp.ad.wrap_discretization(
            self, self._discretization, nd=nd, subdomains=subdomains
        )


## Flow related


class MpfaAd(DiscretizationAd):
    def __init__(self, keyword: str, subdomains: list[pp.Grid], nd: int) -> None:
        self.subdomains = subdomains
        self._discretization = pp.Mpfa(keyword)
        self._name = "Mpfa"
        self.keyword = keyword

        self.flux: Callable[[], pp.ad.MergedOperator]
        self.bound_flux: Callable[[], pp.ad.MergedOperator]
        self.bound_pressure_cell: Callable[[], pp.ad.MergedOperator]
        self.bound_pressure_face: Callable[[], pp.ad.MergedOperator]
        self.vector_source: Callable[[], pp.ad.MergedOperator]
        self.bound_pressure_vector_source: Callable[[], pp.ad.MergedOperator]

        pp.ad.wrap_discretization(
            self, self._discretization, nd=nd, subdomains=subdomains
        )


class TpfaAd(DiscretizationAd):
    def __init__(self, keyword: str, subdomains: list[pp.Grid], nd: int) -> None:
        self.subdomains = subdomains
        self._discretization = pp.Tpfa(keyword)
        self._name = "Tpfa"
        self.keyword = keyword

        self.flux: Callable[[], pp.ad.MergedOperator]
        self.bound_flux: Callable[[], pp.ad.MergedOperator]
        self.bound_pressure_cell: Callable[[], pp.ad.MergedOperator]
        self.bound_pressure_face: Callable[[], pp.ad.MergedOperator]
        self.vector_source: Callable[[], pp.ad.MergedOperator]
        self.bound_pressure_vector_source: Callable[[], pp.ad.MergedOperator]

        pp.ad.wrap_discretization(
            self, self._discretization, nd=nd, subdomains=subdomains
        )


class UpwindAd(DiscretizationAd):
    def __init__(self, keyword: str, subdomains: list[pp.Grid]) -> None:
        self.subdomains = subdomains
        self._discretization = pp.Upwind(keyword)
        self._name = "Upwind"
        self.keyword = keyword

        self.upwind: Callable[[], pp.ad.MergedOperator]
        self.bound_transport_dir: Callable[[], pp.ad.MergedOperator]
        self.bound_transport_neu: Callable[[], pp.ad.MergedOperator]
        # Upwind discretizations always act on scalar fields, so nd=1 is used here.
        pp.ad.wrap_discretization(
            self, self._discretization, nd=1, subdomains=subdomains
        )


class UpwindCouplingAd(DiscretizationAd):
    def __init__(self, keyword: str, interfaces: list[pp.MortarGrid]) -> None:
        self.interfaces = interfaces
        self._discretization = pp.UpwindCoupling(keyword)
        self._name = "Upwind coupling"
        self.keyword = keyword

        # UpwindCoupling also has discretization matrices for (inverse) trace.
        # These are not needed for Ad version since ad.Trace should be used instead
        self.mortar_discr: Callable[[], pp.ad.MergedOperator]
        self.flux: Callable[[], pp.ad.MergedOperator]
        self.upwind_primary: Callable[[], pp.ad.MergedOperator]
        self.upwind_secondary: Callable[[], pp.ad.MergedOperator]
        # Upwind discretizations always act on scalar fields, so nd=1 is used here.
        pp.ad.wrap_discretization(
            self, self._discretization, nd=1, interfaces=interfaces
        )

    def __repr__(self) -> str:
        s = (
            f"Ad discretization of type {self._name}."
            f"Defined on {len(self.interfaces)} interfaces."
        )
        return s
