...
class RobinCoupling(
    ...
    def assemble_matrix_rhs(
        ...
        self.discr_low.assemble_int_bound_source(
            g_low, data_low, data_edge, cc, matrix, rhs, low_ind
        )
    ...
    def assemble_edge_coupling_via_high_dim(
        ...
        if isinstance(mg_primary, pp.BoundaryMortar) and edge_primary[0] == g:
            proj_pressure = mg_primary.low_to_mortar_avg()
...
class FluxPressureContinuity(RobinCoupling):
    ...
    def assemble_matrix_rhs(
        ...
        if g_high.dim == g_low.dim:
            ...
            self.discr_low.assemble_int_bound_flux(
                g_low,
                data_low,
                data_edge,
                cc_low,
            ...
            cc_low[2, low_ind] = -cc_low[2, low_ind]
            rhs_low[2] = -rhs_low[2]
            # Note that cc_low[2, 2] is fliped twice, first for pressure continuity
        else:
