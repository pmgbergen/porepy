...
class Tpsa(Discretization):
    ...
    def __init__(self, keyword: str) -> None:
        ...
        self.rotation_rotation_matrix_key: str = "rotation_rotation"
        """Keyword used to identify the discretization matrix for the rotations
        generated by the cell center rotational stress. Defaults to
        'rotation_rotation'."""
    ...
    def discretize(self, sd: Grid, data: dict) -> None:
        ...
        # central definitions provided in Section 2. The ingredients of the
        # discretization are:
        ...
        mass_total_pressure = -filters.dir_notpass @ (
            ...
            # NOTE: Cell-faces contains a +-1 scaling depending on the direction of the
            # normal vector, reflecting the expression in the paper.
            @ sd.cell_faces
        )
        ...
        if nd == 3:
            ...
            rotation_rotation = (
                ...
                @ Rn_hat
                @ Rn_hat
                # NOTE: Cell-faces contains a +-1 scaling depending on the direction of
            ...
            # Spelled out, from the right: Map the rotation to a vector quantity,
            # multiply with Rn_hat (this amounts to taking the cross project with the
            # normal vector), scale it (including dividing by face_areas, to compensate
            # for the face area scaling of the normal vectors), filter away anything but
            # Dirichlet and Neumann condition, and finally multiply with Rn_bar (which
            # will also bring this back to a face-wise scalar quantity).
            #
            # As an implementation note, the product Rn_bar @ Rn_hat is, by EK's
            # calculation, equal to -1. However, since we need to do the filtering of
            # Neumann and Robin conditions on a vector basis (allowing for different
            # conditions in the two directions), and since Rn_bar is implemented as a
            # vector to scalar map (Rn_hat is not, for some reason, but its product with
            # sd.cell_faces should effectively be so), it is convenient to leave the
            # implementation as it is. Hopefully, things will work out.
            ...
            rotation_rotation = -(
                ...
                # NOTE: Cell-faces contains a +-1 scaling depending on the direction of
                # the normal vector, reflecting the expression in the paper.
                @ sps.kron(sd.cell_faces, sps.csr_matrix([[1], [1]]), format="csr")
            )
        ...
        ## Boundary conditions for the rotation and mass conservation equations.
        #
        # The switching of signs here need some explanations: The paper, equation A2.25
        # has factors \Delta_{T,k}^* (accounting for whether the normal vector points
        # inwards or outwards) for the rotation-rotation and mass-solid-pressure terms
        # and, correspondingly \Delta_{B,k}^* for the boundary terms. Though the T and B
        # terms should have different sign, this change is already incorporated in the
        # transition from A2.23 to A2.25 (see the sign change in row 2 and 3, column 1
        # of the boundary matrix). Hence, the Neumann terms below have different signs
        # compared to the paper.
        #
        # Furthermore, the Neumann condition is assumed set according to an outer normal
        # vector from the user side. Hence, the normal vectors for the R_n^k term in
        # bound_rotation_displacement, and the normal vector in bound_mass_displacement
        # should also be outer. To achive this, we need to flip any inward pointing
        # vectors on the boundary. Combine this flip with the flip in the
        # \Delta_{k,B}^*, and the result is that there is no sign change for the Neumann
        # terms.

        # Need an inverse area for the stress boundary conditions, since both Rn_bar and
        # the normal vectors scale with face areas.
        inv_area_scaling = sps.dia_array(
            (1 / np.repeat(sd.face_areas, nd), 0), shape=(nd * nf, nd * nf)
        )
        # Boundary term for the rotation generated by displacement.
        bound_rotation_displacement = Rn_bar @ (
            -inv_area_scaling @ filters.neu_rob_pass_nd @ inv_mu_face
            - filters.dir_pass_nd
            - c2f_maps.b2f_rob
        )
        ...
        matrix_dictionary[self.rotation_rotation_matrix_key] = rotation_rotation
