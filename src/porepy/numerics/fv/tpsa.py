...
class Tpsa:
    ...
    def discretize(self, sd: Grid, data: dict) -> None:
        ...
        inv_area_scaling = sps.dia_array(
            (np.repeat(1 / sd.face_areas, nd), 0), shape=(nd * nf, nd * nf)
        )

        # Boundary term for the rotation generated by displacement. Compared to the Tpsa
        # paper, there is a factor sgn missing (\Delta_{b,k}^* in the paper) in the
        # Neumann term. This is due to a different sign conventions; in a similar way
        # the Neumann condition for the momentum balance has a sgn multiplication that
        # is not present in the paper (hence, one could say that the rotation term is
        # multiplied with sgn twice).
        ...
        # Boundary condition for the mass conservation equation. See the rotation
        # boundary condition above regarding the sign convention here compared to that
        # in the paper.
