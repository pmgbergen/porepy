...
class Tpsa:
    ...
    def discretize(self, sd: Grid, data: dict) -> None:
        ...
        # Sign change matrix for the Neumann-related terms in
        # bound_rotation_displacement and bound_mass_displacement (just below); see the
        # discretization paper for details.
        sgn_diag_mat = sps.coo_matrix(
            (numbering.sgn_nd, (numbering.fi_expanded, numbering.fi_expanded)),
            shape=(nf * nd, nf * nd),
        ).tocsr()

        # Need an inverse area for the stress boundary conditions, since both Rn_bar and
        # the normal vectors scale with face areas.
        inv_face_area = sps.dia_matrix(
        ...
        bound_rotation_displacement = Rn_bar @ (
            inv_face_area @ filters.neu_rob_pass_nd @ inv_mu_face @ sgn_diag_mat
            - filters.dir_pass_nd
            - c2f_maps.b2f_rob
        )
