...
maximum_operators = pp.ad.Function(pp.ad.functions.maximum, "maximum for operators")
arctan_operators = pp.ad.Function(pp.ad.functions.arctan, "arctan for operators")


class HybridWeightedAverage(Discretization):
    """ """

    def __init__(self, keyword: str = "transport") -> None:
        self.keyword = keyword

        # # not sure i need all the following
        # self.qt_matrix_key = "qt"
        # self.rho_qt_matrix_key = "rho_qt"
        # self.jacobian_qt_matrix_key = "jac_qt"
        # self.qt_adarray = None
        # self.rho_qt_adarray = None
    ...
    def total_flux(
        ...
        z = -self.wrap_grid_attribute(
            mdg.subdomains(), "cell_centers", dim_max
        )  # it sould be ok...
    ...
    def rho_total_flux(
        ...
        for m in np.arange(mixture.num_phases):
            saturation_m = mixture.get_phase(m).saturation
            density_m = mixture.get_phase(m).mass_density(pressure)
            rho_m = self.density_internal_faces(
