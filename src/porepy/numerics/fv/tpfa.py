    ...
    def discretize(self, sd: pp.Grid, data: dict) -> None:
        ...
        if sd.dim == 0:
            ...
            matrix_dictionary[self.vector_source_matrix_key] = sps.csr_matrix(
                (0, sd.num_cells * max(vector_source_dim, 1) * num_components)
            )
            matrix_dictionary[
        ...
        matrix_dictionary[self.bound_flux_matrix_key] = sps.kron(
            bound_flux, eye_comp
        ).tocsr()
        ...
        matrix_dictionary[self.bound_pressure_vector_source_matrix_key] = sps.kron(
            bound_pressure_vector_source, eye_comp
        ).tocsr()
