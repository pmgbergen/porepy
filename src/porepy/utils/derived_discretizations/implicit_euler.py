...
class ImplicitMpfa(pp.Mpfa):
    ...
    def assemble_int_bound_flux(
        self, g, data, data_edge, cc, matrix, rhs, self_ind, use_low_proj=False
    ):
        """
...
class ImplicitUpwindCoupling(pp.UpwindCoupling):
    ...
    def assemble_matrix_rhs(
        ...
        parameter_dictionary_high = data_high[pp.PARAMETERS]
        parameter_dictionary_low = data_low[pp.PARAMETERS]
        lam_flux = data_edge[pp.PARAMETERS][self.keyword]["darcy_flux"]
        dt = parameter_dictionary_high[self.keyword]["time_step"]
        w_high = (
            parameter_dictionary_high.expand_scalars(
                g_high.num_cells, self.keyword, ["advection_weight"]
            )[0]
            * dt
        )
        w_low = (
            parameter_dictionary_low.expand_scalars(
                g_low.num_cells, self.keyword, ["advection_weight"]
            )[0]
            * dt
        )
        ...
        # signs
        div = np.abs(pp.numerics.fv.fvutils.scalar_divergence(g_high))
        ...
        # import pdb
        # pdb.set_trace()
        cc[2, 0] = sps.diags(lam_flux * flag) * hat_P_avg * div.T * sps.diags(w_high)

        # If fluid flux is negative we use the lower value as weight,
        # i.e., T_check * fluid_flux = lambda.
        # we set cc[2, 1] = T_check * fluid_flux
        cc[2, 1] = sps.diags(lam_flux * not_flag) * check_P_avg * sps.diags(w_low)
