...
class FractureDomain(pp.SlightlyCompressibleDataAssigner):
    def __init__(self, g, d):
        ...
        self.data()[pp.keywords.PARAMETERS]["flow"]["aperture"] = aperture
...
def set_sub_problems(gb):
    ...
    for e, d in gb.edges():
        ...
        kn = 1000 / np.mean(d_l[pp.keywords.PARAMETERS]["flow"]["aperture"])
        data = {"normal_diffusivity": kn}
        d[pp.keywords.PARAMETERS] = pp.Parameters(mortar_grid, ["flow"], [data])
        d[pp.keywords.DISCRETIZATION_MATRICES] = {"flow": {}}
