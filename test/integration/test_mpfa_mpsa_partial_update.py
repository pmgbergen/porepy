...
class TestPartialMPFA(unittest.TestCase):
    ...
    def test_inner_cell_node_keyword(self):
        ...
        partial_flux, partial_bound, active_faces = mpfa.mpfa_partial(g, perm, bnd, nodes=nodes_of_cell, inverter='python')
    ...
    def test_one_cell_a_time_node_keyword(self):
        # Update one and one cell, and verify that the result is the same as with a single computation.
        # The test is similar to what will happen with a memory-constrained splitting.
        g = CartGrid([3, 3])
        g.compute_geometry()
        ...
        for ci in range(g.num_cells):
            ...
            partial_flux, partial_bound, active_faces = mpfa.mpfa_partial(g, perm, bnd, nodes=nodes, inverter='python')
