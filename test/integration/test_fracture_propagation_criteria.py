...
class BasicsTest(unittest.TestCase):
    def test_displacement_correlation_2d(self, do_save=False):
        ...
        """
        n_cells = [12, 12]
        critical_sifs = [0.1, 0.1]
        gb_1 = setup_gb.setup_mech(n_cells, 0.5)
        ...
        save = None
        if do_save:
            save = pp.Exporter(gb_2, "solution", folder="results", fixed_grid=False)
        ...
        def set_bc_mech_2(gb):
            set_bc_mech(gb, top_displacement=0.3)

        lhs_2, rhs_2 = propagate_and_update(
            gb_2, faces, discr, set_bc_mech_2, update_apertures
        )
        u = sps.linalg.spsolve(lhs_2, rhs_2)
        faces, _ = pp.displacement_correlation.faces_to_open(gb_2, u, critical_sifs)
        faces = faces[0]
        ...
        lhs_2, rhs_2 = propagate_and_update(
            gb_2, faces, discr, set_bc_mech_3, update_apertures
        )
        u = sps.linalg.spsolve(lhs_2, rhs_2)
        faces, _ = pp.displacement_correlation.faces_to_open(gb_2, u, critical_sifs)
        faces = faces[0]
    ...
    def test_displacement_correlation_2d_internal_fracture(self, do_save=False):
        ...
        """
        n_cells = [12, 12]
        critical_sifs = [0.01, 0.01]
        gb_1 = setup_gb.setup_mech(n_cells, x_start=0, x_stop=0.75)
        gb_2 = setup_gb.setup_mech(n_cells, x_start=0.25, x_stop=0.50)
        #        gb_2_copy = gb_2.copy()
    ...
    def test_displacement_correlation_3d_internal_fracture(self, do_save=False):
        ...
        gb_2 = setup_gb.setup_mech(n_cells, x_start=0.4, x_stop=0.6)
        ...
        def set_bc_mech_2(gb):
            set_bc_mech(gb, top_displacement=0.03)
        ...
        if do_save:
            save = pp.Exporter(
                gb_2, "SIF_propagation_3D", folder="results", fixed_grid=False
            )

        # Check for propagation
        faces, _ = pp.displacement_correlation.faces_to_open(
            gb_2, u, critical_sifs, rm_factor=rm_f
        )
        faces = faces[0]
        save_vtk(save, u, gb_2, 0)
        # Increase boundary displacement, rediscretize, solve and evaluate
