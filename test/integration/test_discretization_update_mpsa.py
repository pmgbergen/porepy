...
class BasicsTest(unittest.TestCase):
    ...
    def test_discretization_and_propagation_3d_large(self):
        """
        Discretize and solve mechanical problem, propagate fracture, update
        discretization and solve anew. Buckets:
        1 premade in its final form
        2 grown in a single step
        """
        n_cells = [8, 8, 8]
        dim_max = 3
        gb_1 = setup_gb.setup_mech(n_cells, .75)
...
#        (nx + 1) * ny * nz + nx * (ny + 1) * nz + nx * ny * nz/2
#        ind = (n_cells[0] + 1) * n_cells[1] * n_cells[2] \
#                + n_cells[0] * (n_cells[1] + 1) * n_cells[2] \
#                + n_cells[0] * n_cells[1] * n_cells[2]/2
#                + 2 * n_cells[1]
#        np.setdiff(original_frac_faces_1, original_frac_faces_2)
        faces = [1410, 1411, 1412, 1413, 1418, 1419, 1420, 1421, 1426, 1427,
                 1428, 1429, 1434, 1435, 1436, 1437]
        # Propagate and update discretizations
        lhs_2, rhs_2 = propagate_and_update(gb_2, faces, discr, set_bc_mech,
                                            update_apertures)
