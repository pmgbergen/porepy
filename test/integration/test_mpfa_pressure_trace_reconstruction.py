...
class MpfaReconstructPressure(unittest.TestCase):
    def test_cart_2d(self):
        ...
        mpfa = pp.Mpfa('flow')
        _, _, grad_cell, grad_bound = mpfa.mpfa(
            g, k, bc, inverter="python"
        )

        grad_bound_known = np.array([[-0.25,  0.  ,  0.  ,  0.  ],
       [ 0.  , -0.25,  0.  ,  0.  ],
       [ 0.  ,  0.  , -0.25,  0.  ],
       [ 0.  ,  0.  ,  0.  , -0.25]])

        grad_cell_known = np.array([[1.],
       [1.],
       [1.],
       [1.]])
    ...
    def test_simplex_3d_dirichlet(self):
        ...
        self.assertTrue(np.all(np.abs(P_f - np.sum(g.face_centers, axis=0) - p0) < 1e-10))
