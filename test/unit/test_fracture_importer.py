...
class TestImport2dCsv(unittest.TestCase):
    ...
    def test_return_frac_id(self):
        ...
        network, fid = pp.fracture_importer.network_2d_from_csv(file_name, skip_header=0, return_frac_id=True)
    ...
    def test_max_num_fracs_keyword(self):
        ...
        network = pp.fracture_importer.network_2d_from_csv(file_name, skip_header=0, max_num_fracs=1)
        known_pts = np.array([[0, 1], [0, 1]])
        self.assertTrue(test_utils.compare_arrays(known_pts, network.pts))
        known_edges = np.array([[0], [1]])
        self.assertTrue(test_utils.compare_arrays(known_edges, network.edges))
        ...
        # Then load no data
        network = pp.fracture_importer.network_2d_from_csv(file_name, skip_header=0, max_num_fracs=0)
        self.assertTrue(network.pts is None)
        self.assertTrue(network.edges is None)
        self.assertTrue(network.domain is None)
        self.assertTrue(network.num_frac == 0)
    ...
    def test_domain_assignment(self):
        ...
        network = pp.fracture_importer.network_2d_from_csv(file_name, skip_header=0, domain=domain)
    ...
    def test_polyline_single_branch(self):
        ...
        network, fid = pp.fracture_importer.network_2d_from_csv(file_name, skip_header=0, polyline=True, return_frac_id=True)
        known_pts = np.array([[0, 1], [0, 1]])
        self.assertTrue(test_utils.compare_arrays(known_pts, network.pts))
        known_edges = np.array([[0], [1]])
        self.assertTrue(test_utils.compare_arrays(known_edges, network.edges))
    ...
    def test_polyline_two_fractures(self):
        ...
        frac_id_1 = 1
        frac_id_2 = 2
        f = np.hstack((np.hstack((frac_id_1 * np.ones(3), frac_id_2 * np.ones(2))).reshape((-1, 1)), p))
        file_name = 'frac.csv'
        np.savetxt(file_name, f, delimiter=',')
...
class TestImport3dElliptic(unittest.TestCase):
    ...
    def test_create_fracture(self):
        ...
        network = pp.fracture_importer.elliptic_network_3d_from_csv(file_name, has_domain=False)
