...
#------------------------------------------------------------------------------#
    ...
    def test_0d_elimination_2_0d(self):
        f1 = np.array([[0, 1],
                       [.5, .5]])
        f2 = np.array([[.5, .5],
                       [0, 1]])
        ...
        # Enforce node orderning because of Python 3.5 and 2.7.
        # Don't do it in general.
        cell_center_1 = np.array([ 5.00000000e-01, 7.50000000e-01, 0])
        cell_center_2 = np.array([ 2.50000000e-01, 7.50000000e-01, 0])
        cell_center_3 = np.array([ 8.75000000e-01, 5.00000000e-01, 0])
        
        for g, d in gb:
            if g.dim == 1:
                if np.allclose(g.cell_centers[:,0], cell_center_1):
                    d['node_number'] = 1
                elif np.allclose(g.cell_centers[:,0], cell_center_2):
                    d['node_number'] = 2
                elif np.allclose(g.cell_centers[:,0], cell_center_3):
                    d['node_number'] = 3
                else:
                    raise ValueError('Grid not found')
