...
class TestAssembler(unittest.TestCase):
    ...
    def test_single_variable(self):
        ...
        for g, d in gb:
            ...
            if g.grid_num == 1:
                d[key + "_" + pp.keywords.DISCRETIZATION] = MockNodeDiscretization(1)
                g1 = g
            else:
                d[key + "_" + pp.keywords.DISCRETIZATION] = MockNodeDiscretization(2)
                g2 = g


        for _, d in gb.edges():
            d[pp.keywords.PRIMARY_VARIABLES] = {key: {"cells": 1,
             'node_couplings': {g1: key, g2: key, pp.keywords.DISCRETIZATION: MockEdgeDiscretization(1, 1)}}}

        general_assembler = pp.Assembler()
        A, b = general_assembler.assemble_matrix_rhs(gb)

        A_known = np.array([[1, 0, 1], [0, 2, 1], [-1, -1, 1]])
        self.assertTrue(np.allclose(A_known, A.todense()))

    def test_single_variable_not_active(self):
        """ A single variable, but make this inactive. This should raise an exception.
        """
        ...
        with self.assertRaises(Exception) as context:
            ...
            A, b = general_assembler.assemble_matrix_rhs(gb, variables="var_11")

        self.assertTrue('need at least one array to concatenate' in str(context.exception))
    ...
    def test_single_variable_multiple_node_discretizations(self):
        ...
        for g, d in gb:
            d[pp.keywords.PRIMARY_VARIABLES] = {key: {"cells": 1}}
            if g.grid_num == 1:
                d[key + "_" + pp.keywords.DISCRETIZATION] = [MockNodeDiscretization(1), MockNodeDiscretization(4)]
                g1 = g
            else:
                d[key + "_" + pp.keywords.DISCRETIZATION] = MockNodeDiscretization(2)
                g2 = g
    ...
    def test_two_variables_no_coupling(self):
        ...
        key_2 = "var_2"
        for g, d in gb:
            d[pp.keywords.PRIMARY_VARIABLES] = {key_1: {"cells": 1},
                                                key_2: {"cells": 1}}
            if g.grid_num == 1:
                d[key_1 + "_" + pp.keywords.DISCRETIZATION] = MockNodeDiscretization(1)
                d[key_2 + "_" + pp.keywords.DISCRETIZATION] = MockNodeDiscretization(2)
            else:
                d[key_1 + "_" + pp.keywords.DISCRETIZATION] = MockNodeDiscretization(3)
                d[key_2 + "_" + pp.keywords.DISCRETIZATION] = MockNodeDiscretization(4)

        for _, d in gb.edges():
            d[pp.keywords.PRIMARY_VARIABLES] = {key_1: {"cells": 1},
                                                key_2: {"cells": 1}}
            d[key_1 + "_" + pp.keywords.DISCRETIZATION] = MockNodeDiscretization(5)
            d[key_2 + "_" + pp.keywords.DISCRETIZATION] = MockNodeDiscretization(6)
    ...
    def test_two_variables_one_active(self):
        ...
        general_assembler = pp.Assembler()
        A, b = general_assembler.assemble_matrix_rhs(gb, variables=[key_2])
    ...
    def test_two_variables_one_active_one_false_active_variable(self):
        ...
        another active variable that are not used in the grid. This should be
        ...
        general_assembler = pp.Assembler()
        A, b = general_assembler.assemble_matrix_rhs(gb, variables=[key_2, 'var_11'])
    ...
    def test_two_variables_coupling_within_node_and_edge(self):
        ...
        for g, d in gb:
            ...
            if g.grid_num == 1:
                d[key_1 + "_" + pp.keywords.DISCRETIZATION] = MockNodeDiscretization(1)
                d[key_2 + "_" + pp.keywords.DISCRETIZATION] = MockNodeDiscretization(2)
                d[key_1 + "_" + key_2 + "_" + pp.keywords.DISCRETIZATION] = MockNodeDiscretization(5)
            else:
                d[key_1 + "_" + pp.keywords.DISCRETIZATION] = MockNodeDiscretization(3)
                d[key_2 + "_" + pp.keywords.DISCRETIZATION] = MockNodeDiscretization(4)
                d[key_2 + "_" + key_1 + "_" + pp.keywords.DISCRETIZATION] = MockNodeDiscretization(6)
        for _, d in gb.edges():
            d[pp.keywords.PRIMARY_VARIABLES] = {key_1: {"cells": 1},
                                                key_2: {"cells": 1}}
            d[key_1 + "_" + pp.keywords.DISCRETIZATION] = MockNodeDiscretization(7)
            d[key_2 + "_" + pp.keywords.DISCRETIZATION] = MockNodeDiscretization(8)
            d[key_1 + "_" + key_2 + "_" + pp.keywords.DISCRETIZATION] = MockNodeDiscretization(6)
            d[key_2 + "_" + key_1 + "_" + pp.keywords.DISCRETIZATION] = MockNodeDiscretization(1)
        ...
        A_2, b = general_assembler.assemble_matrix_rhs(gb, variables=[key_1, key_2])
    ...
    def test_two_variables_coupling_within_node_and_edge_one_active(self):
        ...
        general_assembler = pp.Assembler()
        A, b = general_assembler.assemble_matrix_rhs(gb, variables=key_1)
    ...
    def test_two_variables_coupling_between_node_and_edge(self):
        ...
        for _, d in gb.edges():
            ...
            d[pp.keywords.PRIMARY_VARIABLES] = {key_1: {"cells": 1,
                 'node_couplings': {g1: key_1, g2: key_1, pp.keywords.DISCRETIZATION: MockEdgeDiscretization(1, 2)}},
                    key_2: {"cells": 1}}
            d[key_2 + "_" + pp.keywords.DISCRETIZATION] = MockNodeDiscretization(8)
    ...
    def test_two_variables_coupling_between_node_and_edge_mixed_dependencies(self):
        ...
        for _, d in gb.edges():
            ...
            d[pp.keywords.PRIMARY_VARIABLES] = {key_1: {"cells": 1,
                 'node_couplings': {g1: key_1, g2: key_2, pp.keywords.DISCRETIZATION: MockEdgeDiscretization(1, 2)}},
                    key_2: {"cells": 1}}
            d[key_2 + "_" + pp.keywords.DISCRETIZATION] = MockNodeDiscretization(8)
    ...
    def test_one_and_two_variables_coupling_between_node_and_edge_mixed_dependencies(self):
    ...
    def test_one_and_two_variables_coupling_between_node_and_edge_mixed_dependencies_two_discretizations(self):
        """ One of the nodes has a single variable. A mortar variable depends on a combination
        ...
        for _, d in gb.edges():
            ...
            d[pp.keywords.PRIMARY_VARIABLES] = {key_1: {"cells": 1,
                 'node_couplings': {g1: key_1, g2: key_2, pp.keywords.DISCRETIZATION: [MockEdgeDiscretization(1, 2), MockEdgeDiscretization(2, -4)]}},
                    key_2: {"cells": 1}}
            d[key_2 + "_" + pp.keywords.DISCRETIZATION] = MockNodeDiscretization(8)
    ...
    def test_one_and_two_variables_coupling_between_node_and_edge_mixed_dependencies_two_discretizations_2(self):
        """ One of the nodes has a single variable. A mortar variable depends on a combination
        mixture of the two variables. The mortar variable has two discretizations.
        """
        gb = self.define_gb()
        ...
        for _, d in gb.edges():
            ...
            d[pp.keywords.PRIMARY_VARIABLES] = {key_1: {"cells": 1,
                 'node_couplings': [{g1: key_1, g2: key_2, pp.keywords.DISCRETIZATION: MockEdgeDiscretization(1, 2)},
                                    {g1: key_2, g2: key_2, pp.keywords.DISCRETIZATION: MockEdgeDiscretization(0, 3)}]},
                    key_2: {"cells": 1}}
            d[key_2 + "_" + pp.keywords.DISCRETIZATION] = MockNodeDiscretization(8)
    ...
    def one_variable_one_sided_coupling_between_node_and_edge(self):
        ...
        for _, d in gb.edges():
            ...
            d[pp.keywords.PRIMARY_VARIABLES] = {key_1: {"cells": 1,
                 'node_couplings': {g1: key_1, g2: None, pp.keywords.DISCRETIZATION: MockEdgeDiscretizationOneSided(1, 2)}}}
...
class MockEdgeDiscretization(object):
    ...
    def assemble_matrix_rhs(self, g_master, g_slave, data_master, data_slave, data_edge, local_matrix):
