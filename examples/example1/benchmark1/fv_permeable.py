...
def add_data(gb, domain):
    ...
    for g, d in gb:
        ...
        if g.dim == 2:
            d['k'] = second_order_tensor.SecondOrderTensor(3, kxx)
        else:
            d['k'] = second_order_tensor.SecondOrderTensor(3, kf*kxx)
        ...
        if bound_faces.size == 0 or g.dim== 0:
            continue
...
exporter.export_vtk(gb, 'fv', ["p"], folder='fv_permeable')

# Consistency check
print(np.sum(error.norm_L2(g, d['p']) for g, d in gb))
assert np.isclose(np.sum(error.norm_L2(g, d['p']) for g, d in gb), 35.7250692356)
