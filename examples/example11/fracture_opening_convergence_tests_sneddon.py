...
def cartesian_2d(n_cells, length, height, a, kw={}, fracture=True):
    ...
    gb = pp.meshing.cart_grid(
        fracture_2d(length, height, a, beta, fracture),
        n_cells,
        physdims=[length, height],
    )
...
def simplex_2d(
    h, length, height, a, beta, folder_name, from_gmsh=False, fracture=True, kw={}
):
    """
...
def assign_parameters(gb, t=None):
    ...
    set_bc_mech_tension_sneddon(
        gb, p0=p0, height=height, length=length, beta=beta, aperture=a, t=t, penny=penny
    )
...
if __name__ == "__main__":
    ...
    h = np.array([11, 4, 2])
    #    h = np.array([2, .8])
    h = np.array([3, 2.2, 1.3, 0.65, 0.52])  # , .36])
    h = np.array([3, 2.2, 1.3, 0.8, 0.65])  # .8
    h = np.array([1.3, 0.65, 0.52, 0.36])
    h = np.array([7 / 24])
