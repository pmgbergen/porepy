...
def analytical_displacements(gb, a, sigma=1):
    ...
    frac_ind_2 = np.arange(
        last_ind + n_f + g_h.frac_pairs.shape[1] * dim_h,
    ...
    if not np.isclose(beta, np.pi / 2):
        ...
        x_ind_2 = np.arange(
            n_f + g_h.frac_pairs.shape[1] * dim_h,
            n_f + g_h.frac_pairs.size * dim_h,
            dim_h,
        )
...
def assign_parameters(gb, t=None):
    ...
    if not penny and False:
        set_bc_mech_tension(
            gb, top_tension=p0, t=t, l=length, h=height, fix_faces=False, symmetry=True
        )
    else:
        set_bc_mech_tension_sneddon(
            gb,
...
def compute_eta(g_h):
    ...
    #    else:
    return pp.cg.dist_point_pointset(g_h.face_centers[:, g_h.frac_pairs[0]], center)
...
def run_multiple_and_plot(nc, function):
    ...
    for item in nc:
        ...
        errors.append(
            L2_error(aperture_a, aperture, areas)
        )  # np.sqrt(np.sum(np.power(e, 2)) / np.sum(areas)))
        errors_I.append(
            L2_norm(aperture_a - aperture, areas) / (np.sum(areas) * np.max(aperture_a))
        )
...
def save(
    gb, u, errors, file_name, folder_name, export=True, sort_error=True, sifs=None
):
    if not export:
        return
    for g, d in gb:
        if g.dim == gb.dim_max():
            ...
            disp = u[: g.dim * g.num_cells].reshape((g.dim, g.num_cells), order="F")
            ...
            cell_map = pp.utils.setmembership.ismember_rows(
                np.around(gb.grids_of_dimension(g.dim - 1)[0].cell_centers, n_digits),
...
if __name__ == "__main__":
    ...
    e, e_max, apertures, apertures_a, u, gb, errors, eta = run_multiple_and_plot(
        h, simplex_3d_of_h
    )
    save(gb, u, errors, file_name, folder_name, export=True, sort_error=True)

    sif_errors, sifs, rms, signed = evaluate_sifs_rm_3d(
        a, beta, gb, u, move_tip=False, sigma=p0
    )
