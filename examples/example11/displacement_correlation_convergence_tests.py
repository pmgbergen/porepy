...
def simplex_2d(nx, a, sigma, beta, kw={}):
    ...
    gb = simplex_2d_gb(
        h, length, height, a, beta, folder_name, from_gmsh, fracture=True
    )
...
def solve(gb, dim_h, kw={}):
    ...
    critical_sifs = [1, 0.1, 0.1]  # Arbitrary, unused critical values
    _, sifs = pp.displacement_correlation.faces_to_open(gb, u, critical_sifs, **kw)
    return sifs[0]
...
def evaluate_sifs_rm(a, beta, dim_h, gb, u, p0):
    ...
    if g_l.dim < 2:
        rms = np.unique(
            np.round(g_l.cell_centers[0] - 0.5 + np.sin(beta) * a, 9)
        ) / np.sin(beta)
    ...
    for rc in rms:
        ...
        kw = {"rm": rmin}
        _, s = pp.displacement_correlation.faces_to_open(gb, u, critical_sifs, **kw)
        s = s[0]
...
def evaluate_sifs_rm_3d(a, beta, gb, u, sigma, move_tip=False):
    ...
    rms = pp.cg.dist_point_pointset(tip_face_coo, g_l.cell_centers)
    d_u = pp.displacement_correlation.relative_displacements(
        u, f_c, g_l, cells_l, faces_l, g_h
    )
...
def plot_vs_normalized(g_l, errors, rm, cut_off=100, g_h=None, fc=None):
    ...
    if fc is None:
        ...
        for i in range(len(faces)):
            ...
            hh = np.max(
                pp.cg.dist_point_pointset(
                    g_h.nodes[:, nodes[0]], g_h.nodes[:, nodes[0:]]
                )
            )
            h4.append(hh)
        h = np.mean(h4)
...
if __name__ == "__main__":
    ...
    if not penny:
        for nx in nxs:
            errors, sifs, gb, u = run_multiple_rm(nx, a, simplex=True, beta=beta)
    ...
    nxs = [11]
    #    nx = 10
    rm = np.array([0.5, 1, 1.5, 2, 2.5])
    for nx in nxs:
        h_frac = 1 / (2 * nx)
    errors, sifs, gb, u, rms, signed = run_multiple_rm_3d(
        nx, a, t, simplex=True, beta=beta, sigma=p0
    )
    ...
    ##    print(sifs[0, 0],'\n ')
    ##    print(sifs_n)
    errors, sifs, rms, signed = evaluate_sifs_rm_3d(
        a, beta, gb, u, move_tip=True, sigma=p0
    )
    #
    if rms.ndim < 2:
        rms = rms[:, np.newaxis]
    ...
    print(close_sifs)
    print("errors\n", close_errors)
    save(gb, u, errors, file_name, folder_name, export=True, sort_error=True, sifs=sifs)
