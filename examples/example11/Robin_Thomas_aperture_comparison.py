        ...
        if bound_faces.size > 0:
            ...
            if g.dim > last_ind:
                ...
                dist = pp.cg.dist_point_pointset(
                    np.array([10, 10, 0]), bound_face_centers[:, bottom]
                )
                ind = np.argmin(dist)
...
def evaluate_sifs_from_u(a, beta, gb, u, sigma, move_tip=False, **dc_kw):
    ...
    t1 = time.time()
    _, sifs = pp.displacement_correlation.faces_to_open(gb, u, [1, 1, 1], **dc_kw)
    sifs = sifs[0]
...
if __name__ == "__main__":
    ...
    for i in v:
        if fixed_ntips:
            nx = i
            folder = root + "/fracture_tips_{}/boundary_size_{}".format(
                n_fracture_points, i
            )
        else:
            n_fracture_points = i
            size = int(length / nx)
            folder = root + "/boundary_size_{}/fracture_tips_{}".format(size, i)
        if not os.path.exists(folder):
            os.makedirs(folder)
        file_name = folder + "/geometry"
        if not existing_gb:
            sif_l2, sifs, gb, u, sif_e_loc = run(
                nx, a, t, fn=file_name, beta=beta, sigma=p0
            )
        else:
            ...
            assign_parameters(gb)
            sif_l2, sifs, sif_e_loc = evaluate_sifs_from_u(
                a, beta, gb, u, sigma=p0, rm_factor=1
            )
        ...
        aperture_e_loc = (aperture_analytical - aperture) / np.max(aperture_analytical)
        ...
        if not existing_gb:
            d = {
                "u": u,
                "a": aperture,
                "a_a": aperture_analytical,
                "eta": eta,
                "aperture_L2": aperture_l2,
                "aperture_e_loc": aperture_e_loc,
                "sif_l2": sif_l2,
                "sif_e_loc": sif_e_loc,
                "sifs": sifs,
            }
