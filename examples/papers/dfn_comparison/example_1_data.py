...
def assign_frac_id(gb):
    ...
    for g, d in gb:
        ...
        if np.isclose(np.amin(np.abs(g.nodes[0, :])), 0.5) and \
           np.isclose(np.amax(np.abs(g.nodes[0, :])), 0.5):
            d['frac_id'] = 2*np.ones(g.num_cells, dtype=np.int)

        if np.isclose(np.amin(np.abs(g.nodes[1, :])), 0) and \
           np.isclose(np.amax(np.abs(g.nodes[1, :])), 0):
            d['frac_id'] = 1*np.ones(g.num_cells, dtype=np.int)

        if np.isclose(np.amin(np.abs(g.nodes[2, :])), 0) and \
           np.isclose(np.amax(np.abs(g.nodes[2, :])), 0):
...
def source_f0(x, y, z):
    ...
    val = 0.1*(-0.5-x)*(6*x-16*y*y - \
          (16*x*x*y*y)/(x*x+y*y) + 48*x*y*np.arctan2(y,x)) + \
          0.1*(-0.5 - x)*(16*x*x + (16*x*x*y*y)/(x*x + y*y) + \
          48*x*y*np.arctan2(y,x)) + (1/5.)*(-3*x*x + 8*x*y*y - \
          16*x*x*y*np.arctan2(y,x) - 8*y*(x*x + y*y)*np.arctan2(y,x))
...
def source_f1(x, y, z):
    ...
    val = (3/5.)*(-0.5 - x)*x - (3*x*x)/5. - \
          (24/5.)*np.pi*(-0.5 -x)*x*np.abs(z) + (24/5.)*np.pi*x*x*np.abs(z)
...
def source_f2(x, y, z):
    ...
    val = 2*(-1 + y)*y*(1 + y) + 2*(-1 + y)*(-1 + z)*z + \
          2*y*(-1+ z)*z + 2*(1 + y)*(-1 + z)*z
...
def sol_f0(x, y, z):
    return ((-0.1)*(0.5 + x))*(x*x*x + 8*x*y*(x*x + y*y)*np.arctan2(y,x))

#------------------------------------------------------------------------------#

def sol_f1(x, y, z):
    return (1/10.)*(-0.5 - x)*x*x*x - np.abs(z)*np.pi*(4/5.)*(-0.5 - x)*x*x*x
