...
def small_in_big(c, ny, nsmall):
    ...
    cells = i * k * nsmall + np.tile(np.arange(j * k,
                                               (j + 1) * k),
                                     (k,
                                      1)) + np.outer(np.arange(k),
                                                     np.ones(k)) * nsmall
    return cells
...
cell_volumes_ref = read_csv(main_folder +
                            '/cell_volumes_nx_{}.csv'.format(str(ny_ref)),
                            nc_ref)
...
for i, n in enumerate(ny):
    ...
    reordering = np.concatenate((np.arange(0,
                                           int(n / 2 * n_layer)),
                                 np.arange(n * n_layer,
                                           (n + 1) * n_layer),
                                 np.arange(n / 2 * n_layer,
                                           n * n_layer)))
    ...
    for j, y in enumerate(yfactor):
        for k, d in enumerate(deg):
            ...
            p_eq_coarse = read_csv(equi_folder_coarse +
                                   '/pressures'+appendix_coarse+'.csv', nc)
            t_eq_coarse = read_csv(equi_folder_coarse +
                                   '/tracer'+appendix_coarse+'.csv', nc)

            p_red = read_csv(reduced_folder + '/pressures'
                             + appendix_coarse+'.csv', nc)[reordering]
            t_red = read_csv(reduced_folder + '/tracer'
                             + appendix_coarse+'.csv', nc)[reordering]
...
plt.semilogy([1, 2], [np.power(1 / 2, i) * 3.5e-2 for i in [1, 2]],
             c='k', ls=ls[-1])
plt.xticks(np.arange(len(ny)), ny)
...
plt.xlabel(r'$\frac{1}{h}$')
plt.ylabel(r'$E$', rotation=0)
plt.semilogy([1, 2], [np.power(1 / 2, i) * 1.2e-1 for i in [1, 2]],
             c='k', ls=ls[-1])
