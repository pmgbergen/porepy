...
def low_zones(g):
    ...
    zone_0 = np.logical_and(g.cell_centers[0, :] > 0.5,
                            g.cell_centers[1, :] < 0.5)

    zone_1 = np.logical_and.reduce(tuple([g.cell_centers[0, :] > 0.75,
                                          g.cell_centers[1, :] > 0.5,
                                          g.cell_centers[1, :] < 0.75,
                                          g.cell_centers[2, :] > 0.5]))

    zone_2 = np.logical_and.reduce(tuple([g.cell_centers[0, :] > 0.625,
...
def add_data(gb, data, solver_name):
    ...
    for g, d in gb:
        ...
        if b_faces.size != 0:
            ...
            b_flux = np.logical_and.reduce(tuple(b_face_centers[i, :] <
                                                   0.5 - tol for i in range(3)))
            b_pressure = np.logical_and.reduce(tuple(b_face_centers[i, :] >
                                                  0.75 + tol for i in range(3)))
...
class AdvectiveDataAssigner(pp.ParabolicDataAssigner):
    ...
    def __init__(self, g, data, physics='transport', **kwargs):
        ...
        if b_faces.size > 0:
            ...
            val = 0.5 - self.tol
            self.inflow = np.logical_and.reduce(tuple(b_face_centers[i, :] < val
