...
class Data(object):
        ...
        if self.data["alpha"] != 1:
            for g, _ in self.gb:
                if g.dim == 1:
                    ...
                    if num_nodes < 2:
                        num_nodes = 2
                    g_map[g] = pp.refinement.remesh_1d(g, num_nodes=num_nodes)
                    g_map[g].compute_geometry()

        self.gb = pp.fracs.mortars.replace_grids_in_bucket(self.gb, g_map, {}, self.tol)
    ...
    def write_network(file_name):
        ...
        network += "0,0.5,0.5,0.625,0.5\n"
        network += "0,0.625,0.5,0.75,0.5\n"
        network += "0,0.75,0.5,1,0.5\n"

        network += "1,0.5,0,0.5,0.5\n"
        network += "1,0.5,0.5,0.5,0.625\n"
        network += "1,0.5,0.625,0.5,0.75\n"
        network += "1,0.5,0.75,0.5,1\n"

        network += "2,0.5,0.75,0.625,0.75\n"
        network += "2,0.625,0.75,0.75,0.75\n"
        network += "2,0.75,0.75,1,0.75\n"

        network += "3,0.75,0.5,0.75,0.625\n"
        network += "3,0.75,0.625,0.75,0.75\n"
        network += "3,0.75,0.75,0.75,1\n"

        network += "4,0.5,0.625,0.625,0.625\n"
        network += "4,0.625,0.625,0.75,0.625\n"

        network += "5,0.625,0.5,0.625,0.625\n"
        network += "5,0.625,0.625,0.625,0.75\n"
