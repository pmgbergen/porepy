...
class DarcyModelData(elliptic.EllipticDataAssigner):
    def __init__(self, g, data, **kwargs):
        ...
        if b_faces.size > 0:
            ...
            val = 0.5 - self.tol
            self.b_flux = np.logical_and.reduce(tuple(b_face_centers[i, :] < val
                                                  for i in range(self.max_dim)))

            val = 0.75 + self.tol
            self.b_pressure = np.logical_and.reduce(tuple(b_face_centers[i, :] >
                                              val for i in range(self.max_dim)))
    ...
    def bc_val(self):
        ...
        if b_faces.size > 0:
            ...
            f_faces = b_faces[self.b_flux]
            bc_val[f_faces] = -self.aperture()*self.grid().face_areas[f_faces]
            bc_val[b_faces[self.b_pressure]] = 1
