...
def grid(file_geo, mesh_args, tol, enlarge=None, xi=0.1):
    ...
    if enlarge is not None:
        ...
        if enlarge == "left":
            subdom["points"] = np.array(
                [[domain["xmin"], domain["xmin"]], [domain["ymin"], domain["ymax"]]]
            )
            domain["xmin"] -= xi * delta_x
        elif enlarge == "right":
            subdom["points"] = np.array(
                [[domain["xmax"], domain["xmax"]], [domain["ymin"], domain["ymax"]]]
            )
            domain["xmax"] += xi * delta_x
        elif enlarge == "top":
            subdom["points"] = np.array(
                [[domain["xmin"], domain["xmax"]], [domain["ymax"], domain["ymax"]]]
            )
            domain["ymax"] += xi * delta_y
        elif enlarge == "bottom":
            subdom["points"] = np.array(
                [[domain["xmin"], domain["xmax"]], [domain["ymin"], domain["ymin"]]]
            )
...
def raw_from_csv(file_geo, mesh_args, tol=None):
    ...
    p, e, frac = pp.importer.lines_from_csv(
        file_geo, mesh_args, polyline=True, skip_header=0, return_frac_id=True
    )
