    ...
    for item in os.listdir():
        if os.path.isdir(item):
            ...
            if len(split) < 4:
                continue
            if (
                split[0] in solver_names
                and split[2] in meshes
                and split[4] in time_steps
            ):
    ...
    for run in directories:
        ...
        for item in os.listdir():
            ...
            for fi in range(num_fracs):
                ...
                mean_conc = np.sum(concentrations[hit] * volumes_2d[hit]) / np.sum(
                    volumes_2d[hit]
                )
        ...
        for met in set(all_methods):
            for mesh in set(all_meshes):
                ...
                for fi in range(num_frac_plot):
                    ...
                    if do_plot:
                        plt.title(
                            met + " " + mesh + "fracture " + str(fracs_to_plot[fi])
                        )
        ...
        for met in set(all_methods):
            ...
            for fi in range(num_frac_plot):
                ...
                if do_plot:
                    ...
                    if do_save:
                        path = (
                            "figures/Spatial_convergence_"
                            + met
                            + "_facture_"
                            + str(fracs_to_plot[fi])
                            + ".png"
                        )
                        plt.savefig(path)
                else:
